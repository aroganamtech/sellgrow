...
import { Volume2, MessageSquare, Smartphone, Globe, CheckCircle2, Sparkles, Calendar, } from "lucide-react";
...
export default function SolutionsShowcase() {
    ...
    const [bookingId, setBookingId] = useState("voice");
    const [bookingTitle, setBookingTitle] = useState("AI Assistant");
        ...
        const handleIntersection = (entries) => {
            ...
            entries.forEach((entry) => {
                if (entry.isIntersecting) {
                    const id = entry.target.id.replace("showcase-", "");
                    setActiveTab(id);
                }
            });
        };
    ...
    return (<section id="solutions-showcase" className="pt-8 pb-20 bg-slate-50/20 dark:bg-black/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="sticky top-20 z-30 flex justify-center mb-8 pointer-events-none">
          <div className="pointer-events-auto inline-flex flex-wrap justify-center items-center gap-2 p-2 rounded-full bg-[#080d1a]/95 dark:bg-[#050914]/95 border border-white/15 shadow-[0_20px_50px_rgba(0,0,0,0.7)] backdrop-blur-2xl ring-1 ring-white/20">
            ...
            return (<motion.button key={tab.id} onClick={() => scrollToSection(tab.id)} whileHover={{ y: -3, scale: 1.06 }} whileTap={{ scale: 0.94 }} className={`relative flex items-center gap-2.5 px-5 py-2.5 rounded-full text-xs sm:text-sm font-bold transition-all duration-300 ${isActive
                    ? "text-white shadow-xl"
                    : "text-slate-400 hover:text-white hover:bg-white/10"}`}>
                  {/* Glowing active background pill */}
                  {isActive && (<motion.div layoutId="activeTabPillBackground" className={`absolute inset-0 rounded-full bg-gradient-to-r ${style.bg} ${style.glow} -z-10`} transition={{ type: "spring", stiffness: 500, damping: 35 }}/>)}
        ...
        <div className="space-y-12">
          {solutionsData.map((solution, index) => {
            ...
            return (<SolutionBlock key={solution.id} item={solution} isReversed={isReversed} onBookDemo={openBooking} onOpenSolutionModal={openSolutionModal}/>);
        })}
        </div>

      </div>

      <BookingModal isOpen={isBookingOpen} onClose={() => setIsBookingOpen(false)} solutionId={bookingId} solutionTitle={bookingTitle}/>

      <SolutionModal isOpen={isSolutionModalOpen} onClose={() => setIsSolutionModalOpen(false)} solution={selectedSolution} onBookDemo={openBooking}/>
    </section>);
}
function SolutionBlock({ item, isReversed, onBookDemo, onOpenSolutionModal }) {
    const { t } = useLanguage();
    ...
    return (<motion.div id={`showcase-${item.id}`} initial={{ opacity: 1, y: 0 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }} className="scroll-mt-[150px] bg-white dark:bg-[#0c121e] border border-border rounded-3xl p-6 sm:p-8 md:p-10 shadow-xl transition-all duration-300 hover:shadow-2xl hover:border-border/80">
      <div className={`grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center`}>
        ...
        <div className={`lg:col-span-7 w-full ${isReversed ? "lg:order-1" : "lg:order-2"} flex flex-col gap-4`}>
          <div onClick={handleVideoClick} className="relative rounded-2xl overflow-hidden shadow-2xl border border-border bg-slate-900 aspect-[16/9] cursor-pointer">
            ...
            {/* Video Tag */}
            <video ref={videoRef} className="w-full h-full object-cover pt-9" src={item.videoUrl} autoPlay loop muted playsInline/>
          </div>

          {/* Action Buttons (Solution & Book Demo, aligned to right side under video) */}
          <div className="flex flex-wrap items-center justify-end gap-3 pt-1">
            <button onClick={() => onOpenSolutionModal(item)} className="px-5 py-2.5 bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-xl text-xs font-bold transition-all flex items-center gap-2 transform hover:-translate-y-0.5 active:translate-y-0 shrink-0 border border-slate-200 dark:border-slate-700/60 shadow-sm">
              <Sparkles className="w-3.5 h-3.5 text-primary"/>
              <span>{t("solutionBtn")} - {item.title}</span>
            </button>

            <button onClick={() => onBookDemo(item.id, item.title)} className="px-5 py-2.5 bg-gradient-to-r from-primary to-secondary hover:opacity-95 text-white rounded-xl text-xs font-bold shadow-md shadow-primary/20 transition-all flex items-center gap-2 transform hover:-translate-y-0.5 active:translate-y-0 shrink-0">
