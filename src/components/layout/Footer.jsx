...
import { Github, Twitter, Linkedin, Instagram, Facebook, Youtube, Send } from "lucide-react";
...
export default function Footer() {
    ...
    return (<footer className="border-t border-border/80 bg-white dark:bg-[#060a10] transition-colors duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
          ...
          <div id="footer-features" className="lg:col-span-2 md:col-span-1 space-y-3">
            ...
            <ul className="space-y-2 text-xs text-muted-foreground">
              <li>
                <Link href="/sales-crm" className="hover:text-foreground transition-colors">
                  {t("crm")}
                </Link>
              </li>
              <li>
                <Link href="/unified-inbox" className="hover:text-foreground transition-colors">
                  {t("inbox")}
                </Link>
              </li>
              <li>
          ...
          <div className="lg:col-span-4 md:col-span-2 space-y-3">
            ...
            <form onSubmit={handleSubscribe} className="relative mt-2 max-w-md">
              <input type="text" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={t("footerEnterEmail")} required className="w-full pl-3 pr-10 py-2 text-xs rounded-xl border border-border glass-input focus:outline-none focus:border-primary"/>
              <button type="submit" className="absolute right-1 top-1 bottom-1 px-3 rounded-lg bg-primary hover:opacity-90 text-white transition-all text-xs font-semibold flex items-center justify-center" aria-label="Submit Email">
                {subscribed ? (<span className="text-[10px] font-bold">{t("footerDone")}</span>) : (<Send className="w-3.5 h-3.5"/>)}
              </button>
            </form>
          </div>
        </div>
