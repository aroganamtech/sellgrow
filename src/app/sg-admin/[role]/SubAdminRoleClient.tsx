...
export default function SubAdminRoleClient() {
  ...
  const [activeTab, setActiveTab] = useState<"dashboard" | "services" | "employees" | "flags" | "notifications">("dashboard");
  ...
  const handleToggleNotificationSetting = (key: keyof typeof operatorNotifySettings) => {
  ...
  const DEFAULT_TEAM_STATIC: TeamMember[] = [
  ...
  const DEFAULT_SERVICES_STATIC: ServiceItem[] = [
  ...
  const DEFAULT_EMPLOYEES_STATIC: Employee[] = [
    ...
    const fetchLatestData = () => {
      ...
      fetch("/api/admin/team")
        ...
        .then((data) => {
          if (data.status === "success" && data.data) {
            const me = data.data.find((m: TeamMember) => 
        ...
        .catch(() => {
          ...
          const teamList: TeamMember[] = storedTeam ? JSON.parse(storedTeam) : DEFAULT_TEAM_STATIC;
      ...
      fetch("/api/admin/employees")
        ...
        .catch(() => {
          ...
          const empList: Employee[] = storedEmps ? JSON.parse(storedEmps) : DEFAULT_EMPLOYEES_STATIC;
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    fetch("/api/admin/team")
      .then((res) => {
        if (!res.ok) throw new Error("Offline host");
        return res.json();
      })
      .then((data) => {
        if (data.status === "success" && data.data) {
      ...
      .catch(() => {
        ...
        const match = teamList.find(
          (tm) => 
            (tm.email.toLowerCase() === email.toLowerCase() || email.toLowerCase().startsWith(roleSlug)) && 
  ...
  const navItems: { id: "dashboard" | "products" | "services" | "employees" | "flags" | "notifications"; label: string; icon: any; badge?: number }[] = [
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0a0f1d] flex items-center justify-between px-6 shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={toggleTheme}
              className="p-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-300 hover:text-primary transition-colors flex items-center gap-1.5"
            ...
            <div className="relative">
              <button
                ...
                className="relative p-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-300 hover:text-primary transition-colors flex items-center gap-1.5"
                title="Notifications"
              >
                <Bell className="w-4 h-4" />
                {unreadCount > 0 && (
                  <span className="w-4 h-4 rounded-full bg-red-500 text-white text-[9px] font-extrabold flex items-center justify-center animate-pulse">
                    {unreadCount}
                  </span>
                )}
              </button>
              ...
              {showNotificationPopover && (
                <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl z-50 p-4 space-y-3">
                  ...
                  <button
                    onClick={() => { setActiveTab("notifications"); setShowNotificationPopover(false); }}
                    className="w-full py-2 bg-slate-100 dark:bg-slate-800 hover:bg-primary hover:text-white text-slate-700 dark:text-slate-300 text-xs font-bold rounded-xl transition-colors text-center block"
                  >
              ...
              <div className="p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-xl space-y-6">
                <div className="flex items-center justify-between flex-wrap gap-4 border-b border-slate-150 dark:border-slate-800 pb-5">
                  ...
                  {unreadCount > 0 && (
                    <button
                      onClick={handleMarkAllRead}
                      className="px-3.5 py-2 bg-primary/10 text-primary border border-primary/20 text-xs font-bold rounded-xl hover:bg-primary hover:text-white transition-all"
                    >
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    {Object.entries(operatorNotifySettings).map(([key, enabled]) => (
                      <button
                        key={key}
                        onClick={() => handleToggleNotificationSetting(key as any)}
                      ...
                      >
                        ...
                        <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded-full ${
                          enabled ? "bg-primary text-white" : "bg-slate-200 dark:bg-slate-800 text-slate-400"
                        }`}>
                          {enabled ? "ON" : "OFF"}
                        </span>
                      </button>
