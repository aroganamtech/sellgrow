...
function Hologram3DIcon({ className = "w-5 h-5" }) {
...
function ProductGraphic({ product, is3DHover = false }) {
    return (<div className="relative w-full h-full flex items-center justify-center overflow-hidden transition-all duration-500">
      ...
      <div className="relative z-10 w-full h-full flex flex-col items-center justify-center p-4 transition-transform duration-500">
        {product.category === "BRUSH CUTTER" && (<svg viewBox="0 0 240 180" className="w-4/5 h-4/5 drop-shadow-xl">
            <defs>
        ...
        {product.category === "COMBINE HARVESTER" && (<svg viewBox="0 0 240 180" className="w-4/5 h-4/5 drop-shadow-xl">
            <defs>
        ...
        {(product.category === "POWER TILLER" || product.category === "POWER WEEDER") && (<svg viewBox="0 0 240 180" className="w-4/5 h-4/5 drop-shadow-xl">
            <defs>
        ...
        {product.category === "REAPER" && (<svg viewBox="0 0 240 180" className="w-4/5 h-4/5 drop-shadow-xl">
...
export default function ProductsPage() {
    ...
    const [displayLayout, setDisplayLayout] = useState("marquee_line");
    ...
    const [voiceModelProduct, setVoiceModelProduct] = useState(null);
    const [hologramProduct, setHologramProduct] = useState(null);
    ...
    const handleHologramMouseMove = (e) => {
    ...
    const handleHologramTouchStart = (e) => {
    ...
    const handleHologramTouchMove = (e) => {
        if (!isDraggingHologram || e.touches.length !== 1)
            return;
    ...
    const hologramVideoRef = React.useRef(null);
    ...
    const openSlotBookingModal = (product) => {
    ...
    const [holoDemoTargetProduct, setHoloDemoTargetProduct] = useState(null);
    const handleStartAIDemo = (e, product) => {
        e.stopPropagation();
    ...
    const [holoAiLang, setHoloAiLang] = useState("en");
    ...
    const getSmartAiAnswer = (product, query, lang) => {
        if (!product)
        ...
        if (lang === "hi") {
            ...
            if (isDisp)
                return `${name} का इंजन डिप्लेसमेंट ${disp} है। यह 360-डिग्री झुकाव पर बिना बंद हुए सुचारू रूप से काम करता है।`;
            if (isWeight)
                return `${name} का कुल सूखा वजन (Dry Weight) ${wt} है। इसे बहुत ही हल्का, संतुलित और एर्गोनॉमिक रूप से डिज़ाइन किया गया है ताकि किसान लंबे समय तक बिना थके काम कर सकें।`;
            if (isEngine)
                return `${name} में ${eng} इंजन है। यह 100% शुद्ध पेट्रोल पर चलता है। इसमें अलग से 2T तेल (Oil) मिलाने की कोई आवश्यकता नहीं है, जिससे धुएं का उत्सर्जन नगण्य रहता है और ईंधन की बचत होती है।`;
            if (isPrice)
                return `${name} की अनुमानित शुरुआती कीमत ${price} है। थोक मूल्य, वारंटी और सीधा विक्रेता संपर्क प्राप्त करने के लिए 'Enquire Now' बटन पर क्लिक करें।`;
            if (isCut)
        ...
        if (lang === "ta") {
            ...
            if (isDisp)
                return `${name} இன் எஞ்சின் கொள்ளளவு ${disp} ஆகும்.`;
            if (isWeight)
                return `${name} இன் மொத்த எடை ${wt} ஆகும். கையாளுவதற்கு மிகவும் எளிதானது.`;
            if (isEngine)
                return `${name} எஞ்சின் வகை ${eng} (${disp}) ஆகும். இது தூய்மையான பெட்ரோலில் இயங்குகிறது.`;
            if (isPrice)
                return `${name} இன் தோராயமான விலை ${price} ஆகும். தள்ளுபடி விவரங்களுக்கு 'Enquire Now' பயன்படுத்தவும்.`;
            if (isCut)
        ...
        if (isDisp)
            return `The engine displacement of ${name} is ${disp}.`;
        if (isWeight)
            return `The dry weight of ${name} is ${wt}, featuring a light and balanced ergonomic design.`;
        if (isEngine)
            return `${name} is powered by a ${eng} engine (${disp}). Operates cleanly on pure petrol with no oil mixing required.`;
        if (isPrice)
            return `The estimated starting price for ${name} is ${price}. You can click 'Enquire Now' for wholesale quotation details.`;
        if (isCut)
    ...
    const categoryType = useMemo(() => {
        ...
        if (cat.includes("ELECTRONIC") || name.includes("SMARTPHONE") || name.includes("LAPTOP") || name.includes("WATCH") || name.includes("TV") || name.includes("EARBUDS"))
            return "ELECTRONICS";
        if (cat.includes("HOME") || cat.includes("KITCHEN") || cat.includes("REFRIGERATOR") || cat.includes("WASHING") || name.includes("REFRIGERATOR") || name.includes("OVEN") || name.includes("HEATER"))
            return "HOME_APPLIANCE";
        if (cat.includes("POWER TOOLS") || cat.includes("WASHER") || name.includes("DRILL") || name.includes("WASHER") || name.includes("COMPRESSOR"))
            return "POWER_TOOL";
        if (cat.includes("AUTOMOTIVE") || name.includes("SCOOTER"))
            return "AUTOMOTIVE";
        return "AGRICULTURE";
    ...
    // Open AI Voice Modal
    const openVoiceAssistant = (e, product) => {
        e.stopPropagation();
    ...
    const open3DHologram = (e, product) => {
        e.stopPropagation();
    ...
    const handleSendVoiceQuery = (e) => {
        e.preventDefault();
        if (!userQuery.trim() || !voiceModelProduct)
            return;
        const q = userQuery.trim();
        setUserQuery("");
        const newHistory = [...aiHistory, { sender: "user", text: q }];
        setAiHistory(newHistory);
        setTimeout(() => {
            if (!voiceModelProduct)
                return;
    ...
    return (<>
      ...
      <header className="relative bg-gradient-to-r from-[#0f1f5c] via-indigo-800 to-[#065535] text-white overflow-hidden">
        ...
        ].map((stat, i) => (<div key={i} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/10 border border-white/15 text-xs font-bold text-white backdrop-blur-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        {/* ======================================================== */}
        {viewMode === "catalog" && (<div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                {selectedCompany !== "All" && (<button onClick={() => setSelectedCompany("All")} className="text-xs text-primary font-bold hover:underline flex items-center gap-1 shrink-0">
              ...
              <div className="flex flex-wrap items-center gap-2 bg-white dark:bg-[#0c1322] p-2 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                { id: "GreenField Agri Farms", name: "GreenField Agri", icon: "🌾", count: 14 },
            ].map((comp) => (<button key={comp.id} onClick={() => setSelectedCompany(comp.id)} className={`px-3.5 py-2 text-xs font-bold rounded-xl transition-all duration-200 flex items-center gap-2 ${selectedCompany === comp.id
                    ...
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-mono font-bold ${selectedCompany === comp.id ? "bg-white/20 text-white" : "bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400"}`}>
                      {comp.count}
                    </span>
                  </button>))}
              </div>
              ...
              {selectedCompany === "All" && (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {[
                    {
                ...
                ].map((card) => (<div key={card.id} onClick={() => setSelectedCompany(card.id)} className="group relative p-5 rounded-2xl bg-white dark:bg-[#0c1322] border border-slate-200 dark:border-slate-800 hover:border-primary/40 dark:hover:border-primary/30 cursor-pointer transition-all duration-300 shadow-sm hover:shadow-xl flex flex-col gap-4 overflow-hidden">
              ...
              {selectedCompany !== "All" && (<div className="relative overflow-hidden p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-[#0d1730] to-indigo-950 text-white border border-slate-700/50 shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  ...
                  <button onClick={() => setSelectedCompany("All")} className="relative z-10 px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-xl text-xs font-bold border border-white/15 backdrop-blur-sm transition-all active:scale-95 shrink-0 flex items-center gap-2">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 bg-white dark:bg-[#0c1322] p-3 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
              <div className="flex items-center gap-2 px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700/60 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 shadow-sm min-w-[180px]">
                ...
                <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="bg-transparent font-bold text-slate-700 dark:text-slate-200 focus:outline-none cursor-pointer flex-1 text-xs">
                  {availableCategories.map((cat) => (<option key={cat} value={cat} className="bg-white dark:bg-slate-900 text-foreground">
                      {cat}
                    </option>))}
                </select>
              ...
              <div className="relative flex-1">
                ...
                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search catalog models, brands..." className="w-full pl-10 pr-9 py-2.5 bg-slate-50 dark:bg-slate-800/70 text-xs rounded-xl border border-slate-200 dark:border-slate-700/60 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 transition-all text-foreground placeholder:text-slate-400"/>
                {searchQuery && (<button onClick={() => setSearchQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-700 dark:hover:text-white transition-colors">
              ...
              <button onClick={handleOpenAddBrochure} className="px-5 py-2.5 bg-gradient-to-r from-primary to-indigo-600 hover:from-primary/90 hover:to-indigo-600/90 text-white text-xs font-bold rounded-xl flex items-center justify-center gap-2 transition-all shadow-md shadow-primary/20 active:scale-95 shrink-0" title="Add or Download Product Brochure PDF">
            ...
            {filteredProducts.length === 0 ? (<div className="text-center py-20 bg-white dark:bg-[#0c1322] rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
                ...
                <button onClick={() => { setSelectedCategory("All"); setSearchQuery(""); }} className="inline-flex items-center gap-2 px-5 py-2 bg-primary text-white rounded-xl text-xs font-bold hover:bg-primary/90 transition-colors shadow-md shadow-primary/20 active:scale-95">
                  <RotateCw className="w-3.5 h-3.5"/>
                  Reset Filters
                </button>
              </div>) : (<>
                <div className="flex items-center justify-between">
                  ...
                  {selectedCategory !== "All" && (<span className="text-xs text-muted-foreground">Filtered by: <strong className="text-foreground">{selectedCategory}</strong></span>)}
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                  {filteredProducts.map((product) => {
                    const isHovered = hoveredProductId === product.id;
                    return (<div key={product.id} onClick={() => openProductDetail(product)} onMouseEnter={() => setHoveredProductId(product.id)} onMouseLeave={() => setHoveredProductId(null)} className={`group relative bg-white dark:bg-[#0c1322] border rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 flex flex-col ${isHovered
                        ...
                        <div className="relative w-full aspect-square bg-gradient-to-br from-slate-50 to-slate-100 dark:from-[#0a0f1d] dark:to-[#0f172a] overflow-hidden flex items-center justify-center p-4">
                          ...
                          <div className={`absolute top-3 right-3 z-20 flex flex-col gap-1.5 transition-all duration-200 ${isHovered ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-1 pointer-events-none"}`}>
                            <button onClick={(e) => openVoiceAssistant(e, product)} title="Ask AI Voice Assistant" className="w-8 h-8 rounded-full bg-gradient-to-tr from-primary to-indigo-600 text-white flex items-center justify-center shadow-lg shadow-primary/30 hover:scale-110 transition-transform active:scale-95">
                              <Mic className="w-3.5 h-3.5 stroke-[2.3]"/>
                            </button>
                            <button onClick={(e) => open3DHologram(e, product)} title="3D Hologram View" className="w-8 h-8 rounded-full bg-gradient-to-tr from-indigo-600 to-sky-500 text-white flex items-center justify-center shadow-lg shadow-indigo-500/30 hover:scale-110 transition-transform active:scale-95">
                              <Hologram3DIcon className="w-3.5 h-3.5"/>
                            </button>
                          </div>

                          <div className="relative z-10 w-full h-full flex items-center justify-center">
                            {product.image && !failedImages[product.id] ? (<img src={product.image} alt={product.name} onError={() => setFailedImages((prev) => ({ ...prev, [product.id]: true }))} className={`max-h-full max-w-full object-contain drop-shadow-xl transition-transform duration-500 ${isHovered ? "scale-110" : "scale-100"}`}/>) : (<ProductGraphic product={product} is3DHover={isHovered}/>)}
                          </div>
                        ...
                        <div className="p-4 flex flex-col gap-3 flex-1">
                          <div className="flex-1">
                            ...
                            {product.shortDesc && (<p className="text-[11px] text-muted-foreground mt-1 line-clamp-1 leading-relaxed">
                                {product.shortDesc}
                              </p>)}
                          </div>
        ...
        {/* ======================================================== */}
        {viewMode === "detail" && activeProduct && (<div className="space-y-6">

            {/* Back Button */}
            <button onClick={backToCatalog} className="inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 transition-colors shadow-sm">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              <div className="lg:col-span-3 bg-white dark:bg-[#0b111e] rounded-2xl border border-slate-200 dark:border-slate-800 p-4 space-y-4 shadow-sm max-h-[85vh] overflow-y-auto">
                ...
                <div className="relative">
                  ...
                  <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search catalog models..." className="w-full pl-8 pr-3 py-1.5 bg-slate-100 dark:bg-slate-900 text-xs rounded-xl border border-slate-200 dark:border-slate-800 focus:outline-none focus:border-emerald-500"/>
                </div>
                ...
                return (<div key={cat} className="space-y-1.5">
                        ...
                        return (<button key={item.id} onClick={() => openProductDetail(item)} className={`w-full flex items-center gap-2 text-left px-3 py-2 rounded-xl transition-all ${isSelected
              ...
              <div className="lg:col-span-9 space-y-6">
                ...
                <div className="bg-white dark:bg-[#0c1220] rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                  ...
                  <div className="md:col-span-5 relative w-full aspect-[4/3] bg-slate-50 dark:bg-[#070b14] rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex items-center justify-center p-3">
                    ...
                    {activeProduct.image && !failedImages[activeProduct.id] ? (<img src={activeProduct.image} alt={activeProduct.name} onError={() => setFailedImages((prev) => ({ ...prev, [activeProduct.id]: true }))} className="w-full h-full object-contain p-2 drop-shadow-xl"/>) : (<ProductGraphic product={activeProduct}/>)}
                  </div>
                  ...
                  <div className="md:col-span-7 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-2 w-full">
                      <button onClick={(e) => handleStartAIDemo(e, activeProduct)} className="w-full h-12 px-5 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white rounded-2xl text-xs sm:text-sm font-extrabold shadow-md shadow-emerald-600/20 flex items-center justify-center gap-2.5 transition-all active:scale-95 cursor-pointer">
                        <Sparkles className="w-4.5 h-4.5 text-white shrink-0"/>
                        <span>AI Demo</span>
                      </button>

                      <button onClick={() => openSlotBookingModal(activeProduct)} className="w-full h-12 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl text-xs sm:text-sm font-extrabold shadow-md shadow-emerald-600/20 flex items-center justify-center gap-2.5 transition-all active:scale-95 cursor-pointer">
                ...
                <div className="bg-white dark:bg-[#0c1220] rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm space-y-6">
                  ...
                  <div className="flex border-b border-slate-200 dark:border-slate-800 gap-6 text-xs font-bold">
                    <button onClick={() => setActiveTab("description")} className={`pb-3 border-b-2 transition-colors ${activeTab === "description"
                ? "border-emerald-600 text-emerald-600 dark:text-emerald-400 font-extrabold"
                : "border-transparent text-slate-500 hover:text-slate-900"}`}>
                      Description
                    </button>
                    <button onClick={() => setActiveTab("specifications")} className={`pb-3 border-b-2 transition-colors ${activeTab === "specifications"
                ? "border-emerald-600 text-emerald-600 dark:text-emerald-400 font-extrabold"
                : "border-transparent text-slate-500 hover:text-slate-900"}`}>
                      Specifications
                    </button>
                    <button onClick={() => setActiveTab("related")} className={`pb-3 border-b-2 transition-colors ${activeTab === "related"
                ? "border-emerald-600 text-emerald-600 dark:text-emerald-400 font-extrabold"
                : "border-transparent text-slate-500 hover:text-slate-900"}`}>
                      Related Models
                    </button>
                  ...
                  {/* Tab 1: Description */}
                  {activeTab === "description" && (<div className="space-y-4 text-xs sm:text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
                    ...
                    "ISO 9001 certified George Maijo quality assurance"
                ]).map((item, idx) => (<div key={idx} className="flex items-start gap-2.5">
                  ...
                  {activeTab === "specifications" && (<div className="overflow-x-auto">
                ...
                }).map(([key, val], idx) => (<tr key={key} className={idx % 2 === 0 ? "bg-slate-50 dark:bg-slate-900/60" : "bg-white dark:bg-[#0c1220]"}>
                  ...
                  {activeTab === "related" && (<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      {PRODUCTS_DATA.filter((p) => p.category === activeProduct.category && p.id !== activeProduct.id).slice(0, 3).map((rel) => (<div key={rel.id} onClick={() => openProductDetail(rel)} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 cursor-pointer hover:border-emerald-500 transition-all text-xs space-y-2">
                ...
                <div id="enquiry-form" className="bg-emerald-50/50 dark:bg-[#0a1612] rounded-3xl border border-emerald-500/30 p-6 shadow-sm space-y-4">
                  ...
                  {formSubmitted ? (<div className="p-4 bg-emerald-600 text-white rounded-2xl text-xs font-bold flex items-center gap-3">
                    ...
                    </div>) : (<form onSubmit={handleEnquirySubmit} className="space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div>
                          <label className="text-[11px] font-bold text-foreground block mb-1">Your Name *</label>
                          <input type="text" required placeholder="Full Name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                        </div>

                        <div>
                          <label className="text-[11px] font-bold text-foreground block mb-1">Mobile Number *</label>
                          <input type="tel" required placeholder="+91 98765 43210" value={formData.mobile} onChange={(e) => setFormData({ ...formData, mobile: e.target.value })} className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                        </div>

                        <div>
                          <label className="text-[11px] font-bold text-foreground block mb-1">Email Address</label>
                          <input type="email" placeholder="name@domain.com" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                        </div>

                        <div>
                          <label className="text-[11px] font-bold text-foreground block mb-1">Your Location *</label>
                          <input type="text" required placeholder="City, State" value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                        </div>
                      </div>

                      <button type="submit" className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs shadow-md shadow-emerald-600/20 transition-transform active:scale-95">
                        Submit Request
                      </button>
                    </form>)}
                </div>
            ...
            <div className="relative z-10 flex flex-col items-center space-y-4">
              ...
              <div className="w-full bg-slate-900/90 rounded-2xl border border-cyan-500/30 p-4 space-y-2">
                ...
                <div className="w-full h-2.5 bg-slate-950 rounded-full overflow-hidden border border-cyan-900/60">
                  <div className="h-full bg-gradient-to-r from-cyan-500 via-blue-500 to-indigo-500 transition-all duration-1000 ease-linear rounded-full" style={{ width: `${((5 - holoDemoCountdown) / 5) * 100}%` }}/>
                </div>
              </div>
      ...
      {isVoiceModalOpen && voiceModelProduct && (<div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="relative w-full max-w-lg bg-slate-950 text-white rounded-3xl border border-emerald-500/40 p-6 shadow-2xl space-y-4">
            ...
            }} className="px-3 py-1.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:opacity-90 text-white font-extrabold rounded-xl text-[10px] flex items-center gap-1.5 shadow-md active:scale-95 transition-all" title="Train Voice AI with HuggingFace Dataset">
            ...
            <div className="flex items-center justify-between bg-slate-900 p-2 rounded-xl border border-slate-800 text-xs">
              ...
              <div className="flex gap-1.5 flex-wrap">
                ...
                speakTTS(enMsg, "en");
            }} className={`px-3 py-1.5 rounded-xl font-extrabold text-xs transition-all ${voiceLang === "en"
                ...
                speakTTS(hiMsg, "hi");
            }} className={`px-3 py-1.5 rounded-xl font-extrabold text-xs transition-all ${voiceLang === "hi"
                ? "bg-emerald-500 text-white shadow-md shadow-emerald-500/30"
                : "bg-slate-800 text-slate-400 hover:text-white border border-slate-700"}`}>
                  🇮🇳 हिंदी
                </button>
                ...
                speakTTS(taMsg, "ta");
            }} className={`px-3 py-1.5 rounded-xl font-extrabold text-xs transition-all ${voiceLang === "ta"
                ? "bg-emerald-500 text-white shadow-md shadow-emerald-500/30"
                : "bg-slate-800 text-slate-400 hover:text-white border border-slate-700"}`}>
                  🇮🇳 தமிழ்
                </button>
            ...
            <div className="h-48 overflow-y-auto space-y-2 p-3 bg-slate-900/90 rounded-xl border border-slate-800 text-xs">
              {aiHistory.map((h, i) => (<div key={i} className={`flex ${h.sender === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`p-2.5 rounded-xl max-w-[85%] ${h.sender === "user" ? "bg-emerald-600 text-white" : "bg-slate-800 text-slate-200 border border-slate-700 flex items-start justify-between gap-2"}`}>
                    <span>{h.text}</span>
                    {h.sender === "ai" && (<button onClick={() => speakTTS(h.text, voiceLang)} className="p-1 text-emerald-400 hover:text-white rounded transition-colors shrink-0" title="Read Aloud">
            ...
            <form onSubmit={handleSendVoiceQuery} className="flex gap-2">
              <div className="relative flex-1">
                <input type="text" value={userQuery} onChange={(e) => setUserQuery(e.target.value)} placeholder={voiceLang === "hi"
                ? "प्रश्न पूछें: उदा. 'पावर क्या है?' या 'कीमत क्या है?'"
                : voiceLang === "ta"
                    ? "கேள்வி தட்டச்சு செய்க: எ.கா. 'வணக்கம், எப்படி இருக்கிறீர்கள்?'"
                    : "Ask about specs, engine, power, or price..."} className="w-full pl-3 pr-9 py-2 bg-slate-900 text-white text-xs rounded-xl border border-slate-800 focus:outline-none focus:border-emerald-500 font-medium"/>
                <button type="button" onClick={() => handleStartVoiceListening(voiceLang, (transcript) => {
                setUserQuery(transcript);
                const q = transcript.trim();
                if (!q || !voiceModelProduct)
                    return;
                setUserQuery("");
            ...
            })} className={`absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-lg transition-colors ${isListening ? "bg-rose-500 text-white animate-pulse" : "text-slate-400 hover:text-emerald-400"}`} title="Click to speak your question">
      ...
      {is3DModalOpen && hologramProduct && (<div className="fixed inset-0 z-[100000] flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 backdrop-blur-xl overflow-hidden animate-in fade-in duration-300">
          <div className="relative w-full max-w-7xl h-[92vh] bg-white text-slate-900 rounded-3xl border border-slate-200/90 p-4 sm:p-5 shadow-[0_25px_80px_rgba(0,0,0,0.18)] flex flex-col justify-between overflow-hidden">
            ...
            <div className="relative z-20 flex items-center justify-between pb-3 border-b border-slate-200/80">
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleConnectHolo} className={`px-3.5 py-1.5 rounded-xl text-xs font-extrabold transition-all flex items-center gap-1.5 border shadow-sm active:scale-95 cursor-pointer ${isHoloConnected
                ...
                <button onClick={() => setIs3DModalOpen(false)} className="p-2 text-slate-500 hover:text-slate-900 rounded-xl bg-slate-100 hover:bg-rose-50 hover:text-rose-600 border border-slate-200 transition-all cursor-pointer">
                  <X className="w-5 h-5"/>
                </button>
              </div>
            </div>
            ...
            <div className="relative z-10 flex-1 w-full h-full min-h-0 flex flex-col lg:flex-row items-stretch gap-4 mt-3 overflow-hidden">
              ...
              <div className="relative flex-1 w-full h-full flex flex-col justify-between overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 via-white to-slate-100/90 shadow-inner">
                ...
                <div onMouseDown={handleHologramMouseDown} onMouseMove={handleHologramMouseMove} onMouseUp={handleHologramMouseUp} onMouseLeave={handleHologramMouseUp} onTouchStart={handleHologramTouchStart} onTouchMove={handleHologramTouchMove} onTouchEnd={handleHologramTouchEnd} className={`relative z-10 w-full flex-1 min-h-0 flex items-center justify-center p-4 select-none ${isDraggingHologram ? "cursor-grabbing" : "cursor-grab"}`}>
                ...
                <div className="relative z-30 bg-white/95 backdrop-blur-2xl border-t border-slate-200 p-2.5 flex items-center justify-between shadow-lg shrink-0">
                  <div className="flex items-center gap-2">
                    <button onClick={toggleHologramPlay} className="px-3.5 py-1.5 bg-cyan-500 hover:bg-cyan-600 text-white rounded-xl border border-cyan-400 transition-all flex items-center gap-1.5 text-xs font-extrabold shadow-md shadow-cyan-500/20 active:scale-95 cursor-pointer" title={isHologramPlaying ? "Pause Model" : "Play Model"}>
                    ...
                    <button onClick={toggleHologramMute} className={`px-3.5 py-1.5 rounded-xl border transition-all flex items-center gap-1.5 text-xs font-extrabold active:scale-95 cursor-pointer ${!isHologramMuted
                ? "bg-emerald-50 text-emerald-700 border-emerald-300 shadow-sm"
                : "bg-slate-100 text-slate-600 border-slate-200 hover:text-slate-900"}`} title={isHologramMuted ? "Unmute Audio" : "Mute Audio"}>
                    ...
                    <button onClick={() => setIs360Rotating(!is360Rotating)} className={`px-3.5 py-1.5 rounded-xl border transition-all flex items-center gap-1.5 text-xs font-extrabold active:scale-95 cursor-pointer ${is360Rotating
                ? "bg-indigo-50 text-indigo-700 border-indigo-300 shadow-sm"
                : "bg-slate-100 text-slate-600 border-slate-200 hover:text-slate-900"}`} title="Toggle 360° Orbit View">
              ...
              {isHoloAiPanelOpen && (<div className="w-full lg:w-[360px] xl:w-[390px] shrink-0 h-full flex flex-col justify-between bg-white/95 backdrop-blur-2xl border border-slate-200/90 rounded-2xl p-4 shadow-lg text-slate-900 animate-in slide-in-from-right duration-300 space-y-3 overflow-hidden">
                  <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      <button onClick={() => setHoloActiveTab("specs")} className={`px-2.5 py-1 rounded-xl text-[10.5px] font-extrabold transition-all cursor-pointer ${holoActiveTab === "specs"
                    ? "bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-md shadow-cyan-500/20"
                    : "bg-slate-100 text-slate-600 hover:text-slate-900 border border-slate-200"}`}>
                        {holoAiLang === "hi" ? "विवरण" : holoAiLang === "ta" ? "விவரங்கள்" : "Specs"}
                      </button>
                      <button onClick={() => setHoloActiveTab("chat")} className={`px-2.5 py-1 rounded-xl text-[10.5px] font-extrabold transition-all cursor-pointer relative ${holoActiveTab === "chat"
                    ? "bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-md shadow-cyan-500/20"
                    : "bg-slate-100 text-slate-600 hover:text-slate-900 border border-slate-200"}`}>
                        {holoAiLang === "hi" ? "AI पूछें" : holoAiLang === "ta" ? "AI கேட்க" : "Ask AI"}
                      </button>
                      <button onClick={() => setIsHoloAiPanelOpen(false)} className="p-1 text-slate-500 hover:text-slate-900 rounded-lg bg-slate-100 hover:bg-rose-50 hover:text-rose-600 border border-slate-200 transition-colors cursor-pointer ml-1" title="Close AI Assistant Panel">
                        <X className="w-4 h-4"/>
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl border border-slate-200">
                    <button onClick={() => handleHoloLangChange("en")} className={`flex-1 py-1.5 px-2 rounded-lg font-extrabold text-[10.5px] transition-all cursor-pointer ${holoAiLang === "en"
                    ? "bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-md shadow-cyan-500/20"
                    : "text-slate-600 hover:text-slate-900"}`}>
                      🇺🇸 English
                    </button>

                    <button onClick={() => handleHoloLangChange("hi")} className={`flex-1 py-1.5 px-2 rounded-lg font-extrabold text-[10.5px] transition-all cursor-pointer ${holoAiLang === "hi"
                    ? "bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-md shadow-cyan-500/20"
                    : "text-slate-600 hover:text-slate-900"}`}>
                      🇮🇳 हिंदी
                    </button>

                    <button onClick={() => handleHoloLangChange("ta")} className={`flex-1 py-1.5 px-2 rounded-lg font-extrabold text-[10.5px] transition-all cursor-pointer ${holoAiLang === "ta"
                    ? "bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-md shadow-cyan-500/20"
                    : "text-slate-600 hover:text-slate-900"}`}>
                      🇮🇳 தமிழ்
                    </button>
                  ...
                  {holoActiveTab === "specs" ? (<div className="flex-1 flex flex-col justify-between space-y-3 min-h-0 overflow-y-auto">
                    ...
                    }} className={`w-full py-2.5 px-3 rounded-xl text-xs font-extrabold flex items-center justify-center gap-2 shadow-md active:scale-95 cursor-pointer mt-auto transition-all ${isSpeaking
      ...
      {isTamilTrainingModalOpen && (<div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 bg-slate-950/92 backdrop-blur-xl overflow-y-auto animate-in fade-in duration-300">
          <div className="relative w-full max-w-2xl bg-[#030712] text-white rounded-3xl border border-purple-500/40 p-6 sm:p-7 shadow-[0_0_90px_rgba(168,85,247,0.25)] space-y-5 overflow-hidden">
            ...
            <div className="relative z-10 flex items-center justify-between border-b border-purple-900/40 pb-4">
              ...
              <button onClick={() => setIsTamilTrainingModalOpen(false)} className="p-2 text-slate-400 hover:text-white rounded-xl bg-slate-900/80 hover:bg-rose-500/20 border border-slate-800 transition-all">
                <X className="w-5 h-5"/>
              </button>
            </div>
            ...
            <div className="relative z-10 space-y-4">
              ...
              <div className="p-4 rounded-2xl bg-slate-900/90 border border-purple-500/30 space-y-3">
                ...
                <input type="text" value={hfDatasetUrl} onChange={(e) => setHfDatasetUrl(e.target.value)} className="w-full px-3.5 py-2.5 bg-slate-950 text-white font-mono text-xs rounded-xl border border-purple-500/30 focus:outline-none focus:border-purple-400" placeholder="https://huggingface.co/datasets/..."/>
              </div>
              ...
              {isTrainingActive && (<div className="p-4 rounded-2xl bg-purple-950/40 border border-purple-500/40 space-y-3 animate-pulse">
                  <div className="flex items-center justify-between text-xs font-mono">
              ...
              {isTrainingComplete && (<div className="p-4 rounded-2xl bg-emerald-950/40 border border-emerald-500/40 flex items-center justify-between text-xs text-emerald-300 font-bold">
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className="w-5 h-5 text-emerald-400"/>
                    <span>Tamil AI Voice Speech Model Trained &amp; Synced with ElevenLabs!</span>
                  </div>
                  <button onClick={() => speakTTS("Achitha/simple_tamil தரவுத்தளத்தின் மூலம் AI குரல் முகவர் வெற்றிகரமாக பயிற்சி பெற்றுள்ளது!", "ta")} className="px-3 py-1.5 bg-emerald-500 hover:bg-emerald-400 text-white rounded-xl text-xs font-extrabold flex items-center gap-1.5 shadow-md active:scale-95">
                    <Volume2 className="w-3.5 h-3.5"/>
            ...
            <div className="relative z-10 flex items-center justify-between pt-3 border-t border-purple-900/40">
              <button onClick={() => setIsTamilTrainingModalOpen(false)} className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-800 rounded-xl text-xs font-extrabold transition-all">
                Close Studio
              </button>

              <button onClick={startTamilVoiceTraining} disabled={isTrainingActive} className="px-5 py-2.5 bg-gradient-to-r from-purple-600 via-indigo-600 to-purple-600 hover:opacity-90 disabled:opacity-50 text-white rounded-xl text-xs font-extrabold flex items-center gap-2 shadow-lg shadow-purple-950/50 transition-all active:scale-95">
      ...
      {isHoloDeviceModalOpen && hologramProduct && (<div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-xl animate-in fade-in duration-200">
          <div className="relative w-full max-w-md bg-slate-950 text-white rounded-3xl border border-sky-500/40 p-6 shadow-[0_0_60px_rgba(56,189,248,0.2)] space-y-5">
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setIsHoloDeviceModalOpen(false)} className="p-1.5 text-slate-400 hover:text-white rounded-full hover:bg-slate-800">
                <X className="w-5 h-5"/>
              </button>
            </div>
            ...
            <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 space-y-3">
              ...
              <div className="space-y-2 pt-1">
                {/* Device 1 */}
                <div onClick={handleStartHoloSync} className={`p-3 rounded-xl border text-xs cursor-pointer transition-all flex items-center justify-between ${isHoloConnected
                ...
                : "bg-slate-950/80 border-slate-800 hover:border-sky-500/40 text-slate-200"}`}>
                  ...
                  {isHoloConnecting ? (<span className="text-[10px] text-sky-400 animate-pulse font-mono font-bold">Connecting...</span>) : isHoloConnected ? (<span className="text-[10px] bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full font-bold">CONNECTED</span>) : (<span className="text-[10px] bg-sky-500/20 text-sky-300 px-2.5 py-1 rounded-lg font-bold hover:bg-sky-500/30">Connect</span>)}
                </div>
            ...
            <div className="flex justify-end pt-1">
              <button onClick={() => setIsHoloDeviceModalOpen(false)} className="px-5 py-2 bg-sky-600 hover:bg-sky-500 text-white font-bold rounded-xl text-xs transition-all shadow-lg">
                Done
              </button>
            </div>
      ...
      {isSlotModalOpen && slotProduct && (<div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden p-6 sm:p-8 space-y-6">
            ...
            <div className="flex items-start justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
              ...
              <button onClick={() => setIsSlotModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors">
                <X className="w-5 h-5"/>
              </button>
            </div>
            ...
            <div className="py-6 text-center space-y-4 animate-in zoom-in-95">
                ...
                <div className="pt-2 flex items-center justify-center gap-3">
                  <a href={`https://wa.me/919444154944?text=${encodeURIComponent(`Hi George Maijo Team, I just booked a slot for ${slotProduct.name} on ${slotDate} at ${slotTime}. My name is ${slotForm.name}.`)}`} target="_blank" rel="noopener noreferrer" className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-md transition-all">
                    <MessageSquare className="w-4 h-4"/>
                    <span>Notify on WhatsApp</span>
                  </a>
                  <button onClick={() => setIsSlotModalOpen(false)} className="px-5 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl text-xs font-bold transition-all">
                    Done
                  </button>
                </div>
              </div>) : (
            ...
            <form onSubmit={(e) => {
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    "02:00 PM - 02:45 PM",
                    "04:00 PM - 04:45 PM",
                ].map((t) => (<button key={t} type="button" onClick={() => setSlotTime(t)} className={`py-2 px-3 rounded-xl text-xs font-bold transition-all border text-center ${slotTime === t
                ...
                <div className="space-y-3 pt-1 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <input type="text" required placeholder="e.g. Ramesh Kumar" value={slotForm.name} onChange={(e) => setSlotForm({ ...slotForm, name: e.target.value })} className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 text-xs rounded-xl border border-slate-200 dark:border-slate-700 focus:outline-none focus:border-emerald-500 font-medium"/>
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400">Phone Number *</label>
                      <input type="tel" required placeholder="+91 98765 43210" value={slotForm.phone} onChange={(e) => setSlotForm({ ...slotForm, phone: e.target.value })} className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 text-xs rounded-xl border border-slate-200 dark:border-slate-700 focus:outline-none focus:border-emerald-500 font-medium"/>
                    </div>
                  </div>

                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400">Email Address (Optional)</label>
                    <input type="email" placeholder="ramesh@example.com" value={slotForm.email} onChange={(e) => setSlotForm({ ...slotForm, email: e.target.value })} className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 text-xs rounded-xl border border-slate-200 dark:border-slate-700 focus:outline-none focus:border-emerald-500 font-medium"/>
                  </div>
                </div>

                {/* Submit button */}
                <button type="submit" className="w-full py-3 px-4 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-extrabold rounded-2xl text-xs sm:text-sm shadow-lg shadow-emerald-600/30 flex items-center justify-center gap-2 transition-all active:scale-98 cursor-pointer">
                  <Calendar className="w-4 h-4"/>
