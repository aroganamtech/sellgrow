...
import { Users, DollarSign, Zap, Settings, FileText, Search, RefreshCw, ToggleLeft, ToggleRight, Sparkles, LayoutDashboard, CreditCard, Cpu, User, LogOut, Menu, X, ChevronLeft, ChevronRight, Check, ShieldAlert, Key, Plus, Trash, Sliders, Lock, ExternalLink, Crown, Sun, Moon, Edit, Package, Trash2, Bell, BellRing, FilePlus, Upload, CheckCircle2 } from "lucide-react";
...
export default function AdminGrowthPage() {
    ...
    const [teamsActiveSubTab, setTeamsActiveSubTab] = useState("sub-admin");
    ...
    const [invitationSuccessModal, setInvitationSuccessModal] = useState(null);
    const [selectedSubAdminModal, setSelectedSubAdminModal] = useState(null);
    ...
    const handleInviteTeamMember = (e) => {
        e.preventDefault();
        if (!newTeamMemberName || !newTeamMemberEmail)
            return;
    ...
    const handleSaveSubAdminDetails = (member) => {
    ...
    const handleAssignService = (memberId, serviceId, checked) => {
        setTeamMembers(prev => prev.map(m => {
            if (m.id !== memberId)
                return m;
            const current = m.assignedServices || [];
        ...
        if (!member)
            return;
        const current = member.assignedServices || [];
        const updated = checked
            ? [...current.filter((s) => s !== serviceId), serviceId]
            : current.filter((s) => s !== serviceId);
        fetch("/api/admin/team", {
    ...
    const [sgTermTab, setSgTermTab] = useState("terminal");
    const [legalTermsType, setLegalTermsType] = useState("tos");
    const [legalTosContent, setLegalTosContent] = useState("Welcome to SellGrow. These Terms of Service ('Terms') govern your access to and use of SellGrow's AI agent building suite, automated WhatsApp pipelines, and platform services. By using our website, you agree to comply with all local, state, and international regulations regarding digital communications and AI agent implementations. System abuse will lead to immediate key revocation.");
    const [legalPrivacyContent, setLegalPrivacyContent] = useState("SellGrow is committed to user privacy. We secure all API key vaults, databases, and AI chat transcripts with AES-256 encryption. Telemetry reports, Geo-IP classification data, and communication logs are stored securely on authorized AWS instances. No PII is shared with outside third-party marketing firms.");
    const [legalSlaContent, setLegalSlaContent] = useState("SellGrow guarantees a 99.9% uptime for core platform services including API gateways, WhatsApp triggers, and AI agent execution pipelines. SLA credits are calculated monthly. Maintenance operations are scheduled during low-traffic windows (2:00 AM - 4:00 AM IST) and announced 48 hours in advance.");
    const [saveToast, setSaveToast] = useState(false);
    // Super Admin Product Catalog State
    const [adminCatalogProducts, setAdminCatalogProducts] = useState(PRODUCTS_DATA);
    ...
    const handleAnalyzePdfBrochure = async (fileNameInput) => {
        ...
        const analysis = ProductPdfIntelligenceModel.analyzePdfBrochure(targetPdf, brochureData.name, "George Maijo Agri");
    ...
    const handleSaveAddBrochure = async (e) => {
        ...
        const finalSpecs = { ...brochureData.specs };
                ...
                if (stored) {
                    try {
                        ...
                        if (Array.isArray(parsed) && parsed.length > 0) {
                            const customProducts = parsed.filter((p) => !deletedIds.includes(p.id));
    ...
    const saveCatalogState = async (newProducts) => {
    ...
    const handleRemoveProductImage = async (id) => {
    ...
    const [subAdminActiveTab, setSubAdminActiveTab] = useState("super-admin");
    const [openScopePopover, setOpenScopePopover] = useState(null);
    const [showInviteModal, setShowInviteModal] = useState(false);
    const [showEmployeeModal, setShowEmployeeModal] = useState(false);
    const [inviteModalRole, setInviteModalRole] = useState("SubAdmin");
    const [expandedSubAdminId, setExpandedSubAdminId] = useState(null);
    const [expandedEmployeeId, setExpandedEmployeeId] = useState(null);
    const [serviceScopeModalTarget, setServiceScopeModalTarget] = useState(null);
    ...
    const toggleUserNotification = (id) => {
    ...
    const handleTerminalSubmit = (e) => {
        e.preventDefault();
        if (!developerLogTerminalInput.trim())
            return;
    ...
    if (isLoading || !isAuthenticated) {
        return (<div className="min-h-screen bg-[#070b15] flex flex-col items-center justify-center text-slate-400 text-sm gap-3">
    ...
    return (<div className="min-h-screen bg-slate-50 dark:bg-[#070b13] text-slate-800 dark:text-slate-100 flex font-sans transition-colors duration-300">
      
      {/* 1. LEFT SIDEBAR (SLIDER BAR) */}
      {/* Drawer Overlay for Mobile */}
      <AnimatePresence>
        {mobileMenuOpen && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setMobileMenuOpen(false)} className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm lg:hidden"/>)}
      </AnimatePresence>

      {/* Sidebar Panel */}
      <aside className={`fixed top-0 bottom-0 left-0 z-50 flex flex-col bg-white dark:bg-[#0a0f1d] border-r border-slate-200 dark:border-slate-800/80 shadow-2xl transition-all duration-300
        ...
        <div className="p-5 flex items-center justify-between border-b border-slate-100 dark:border-slate-800/50">
          ...
          {/* Close drawer (Mobile) */}
          <button onClick={() => setMobileMenuOpen(false)} className="lg:hidden p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">
            <X className="w-5 h-5"/>
          </button>
        </div>

        {/* Sidebar Mini Profile Block */}
        {!sidebarCollapsed && (<div className="p-4 mx-4 my-3 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-150 dark:border-slate-800/40 flex items-center gap-3">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {menuItems.map((item) => {
            ...
            return (<button key={item.id} onClick={() => {
                ...
                {!sidebarCollapsed && (<motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 text-left">
                    {item.label}
                  </motion.span>)}
                {/* Active sidebar bar indicator */}
                {isActive && (<div className="absolute right-0 top-1/4 bottom-1/4 w-1.5 bg-white rounded-l-md"/>)}
                {/* Custom Badge */}
                {item.badge && !sidebarCollapsed && (<span className="px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-[9px] text-slate-700 dark:text-slate-350 font-extrabold scale-90">
                    {item.badge}
                  </span>)}
              </button>);
        })}
        ...
        <div className="p-3 border-t border-slate-100 dark:border-slate-800/50">
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-xs font-bold text-red-655 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 transition-all group">
        ...
        <div className="hidden lg:block p-3 text-right">
          <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="p-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 text-slate-550 hover:text-slate-900 dark:hover:text-white transition-colors" title={sidebarCollapsed ? "Expand Sidebar" : "Collapse Sidebar"}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        <header className="sticky top-0 z-40 bg-white/95 dark:bg-[#070b13]/95 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800/80 px-6 py-4 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setMobileMenuOpen(true)} className="lg:hidden p-2 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-205 dark:border-slate-800 hover:bg-slate-202 dark:hover:bg-slate-800">
          ...
          <div className="flex items-center gap-2.5">
            ...
            {/* Public Site */}
            <button onClick={() => router.push("/")} className="px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300 transition-colors flex items-center gap-1.5">
              <span className="hidden sm:inline">Public Site</span>
            </button>

            {/* Lock Button */}
            <button onClick={handleLogout} className="px-3 py-2 rounded-xl border border-red-500/30 bg-red-500/10 hover:bg-red-500/20 text-xs font-bold text-red-600 dark:text-red-400 transition-colors flex items-center gap-1.5">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 max-w-7xl w-full mx-auto space-y-8">
          ...
          {/* ======================================= */}
          {activeView === "dashboard" && (<div className="space-y-6">
              {/* 3 Stat Cards */}
                ...
                <div className="p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-xl space-y-6">
                  ...
                  <div className="h-48 w-full relative">
                    <svg className="w-full h-full" viewBox="0 0 600 160" fill="none">
                      ...
                      <path d="M 50,130 L 550,130" stroke="#3b82f6" strokeWidth="2.5" strokeLinecap="round" strokeDasharray="4 4" opacity="0.6"/>
                ...
                <div className="p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-xl space-y-6">
                  ...
                  <div className="h-48 w-full relative">
                    <svg className="w-full h-full" viewBox="0 0 600 160" fill="none">
                      ...
                      {/* Dynamic Baseline (flat $0 chart) */}
                      <path d="M 50,130 L 550,130" stroke="#d946ef" strokeWidth="2.5" strokeLinecap="round" strokeDasharray="4 4" opacity="0.6"/>
          ...
          {/* ======================================= */}
          {activeView === "sub-admins" && (<div className="space-y-6">
              {/* Notification Toast Confirmation */}
              {notificationToast && (<div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-4 py-3 bg-slate-900 text-white text-xs font-bold rounded-2xl shadow-2xl border border-slate-700 animate-pulse">
                  <BellRing className="w-4 h-4 text-emerald-400 shrink-0"/>
                  <span>{notificationToast}</span>
                </div>)}

              {/* Direct Notification Modal for Super Admin, Sub Admin, or Employee */}
              {notificationModalTarget && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setNotificationModalTarget(null)}>
                  <div className="absolute inset-0 bg-black/40 backdrop-blur-sm"/>
                  <div className="relative w-full max-w-md p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-2xl space-y-5" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => setNotificationModalTarget(null)} className="absolute top-4 right-4 p-1.5 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                      <X className="w-4 h-4"/>
                    </button>
                    ...
                    <form onSubmit={handleSendNotification} className="space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1.5">
                          ...
                          <select value={notificationType} onChange={(e) => setNotificationType(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <select value={notificationChannel} onChange={(e) => setNotificationChannel(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input type="text" value={notificationSubject} onChange={(e) => setNotificationSubject(e.target.value)} placeholder="Enter notification subject..." className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-primary" required autoFocus/>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <textarea rows={3} value={notificationMessage} onChange={(e) => setNotificationMessage(e.target.value)} placeholder="Enter message details for this account..." className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-primary resize-none" required/>
                      </div>

                      <button type="submit" className="w-full py-2.5 bg-primary text-white text-xs font-bold rounded-xl hover:opacity-95 transition-opacity shadow-md shadow-primary/10 flex items-center justify-center gap-2">
              ...
              {serviceScopeModalTarget && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setServiceScopeModalTarget(null)}>
                  <div className="absolute inset-0 bg-black/40 backdrop-blur-sm"/>
                  <div className="relative w-full max-w-md max-h-[85vh] p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-2xl flex flex-col space-y-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => setServiceScopeModalTarget(null)} className="absolute top-4 right-4 p-1.5 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors z-10">
                      <X className="w-4 h-4"/>
                    </button>
                    ...
                    const assigned = memberInState.assignedServices || [];
                    const isChecked = assigned.includes(srv.id);
                    return (<label key={srv.id} className={`flex items-center justify-between p-3.5 rounded-2xl border transition-all cursor-pointer ${isChecked
                            ...
                            : "border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-950"}`}>
                              ...
                              <input type="checkbox" className="sr-only" checked={isChecked} onChange={(e) => handleAssignService(serviceScopeModalTarget.id, srv.id, e.target.checked)}/>
                              <div>
                            ...
                            {isChecked && (<span className="px-2 py-0.5 rounded-full text-[9px] font-extrabold bg-emerald-500/10 text-emerald-500 border border-emerald-500/20">
                                Active Scope
                              </span>)}
                          </label>);
                })}
                    ...
                    <button onClick={() => setServiceScopeModalTarget(null)} className="w-full py-2.5 bg-primary text-white text-xs font-bold rounded-xl shadow-lg shadow-primary/25 hover:opacity-90 transition-opacity shrink-0">
                      Save Service Scope
                    </button>
                  </div>
                </div>)}
              ...
              <div className="flex flex-wrap items-center gap-2 pb-1 border-b border-slate-200/80 dark:border-slate-800/80">
                <button onClick={() => setSubAdminActiveTab("super-admin")} className={`px-3.5 sm:px-4 py-2 text-xs font-bold rounded-xl transition-all ${subAdminActiveTab === "super-admin"
                ...
                <button onClick={() => setSubAdminActiveTab("sub-admins")} className={`px-3.5 sm:px-4 py-2 text-xs font-bold rounded-xl transition-all ${subAdminActiveTab === "sub-admins"
                ? "bg-primary text-white shadow-md shadow-primary/20"
                : "bg-slate-50 dark:bg-slate-900/60 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-800 hover:text-slate-900 dark:hover:text-white"}`}>
                  Sub Admin
                </button>
                <button onClick={() => setSubAdminActiveTab("employees")} className={`px-3.5 sm:px-4 py-2 text-xs font-bold rounded-xl transition-all ${subAdminActiveTab === "employees"
                ? "bg-primary text-white shadow-md shadow-primary/20"
                : "bg-slate-50 dark:bg-slate-900/60 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-800 hover:text-slate-900 dark:hover:text-white"}`}>
                  Employees
                </button>
              ...
              {/* Invite User Modal */}
              {showInviteModal && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" onClick={() => setShowInviteModal(false)}>
                  <div className="absolute inset-0 bg-black/40 backdrop-blur-sm"/>
                  <div className="relative w-full max-w-sm p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-2xl space-y-6" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => setShowInviteModal(false)} className="absolute top-4 right-4 p-1.5 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                      <X className="w-4 h-4"/>
                    </button>
                    ...
                    <form onSubmit={(e) => { handleInviteTeamMember(e); setShowInviteModal(false); }} className="space-y-4">
                      <div className="space-y-1.5">
                        ...
                        <input type="text" value={newTeamMemberName} onChange={(e) => setNewTeamMemberName(e.target.value)} placeholder="Enter name..." className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-primary" required autoFocus/>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input type="email" value={newTeamMemberEmail} onChange={(e) => setNewTeamMemberEmail(e.target.value)} placeholder="Enter email..." className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-primary" required/>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <select value={newTeamMemberRole} onChange={(e) => setNewTeamMemberRole(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary">
                      ...
                      <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800">
                        ...
                        <input type="checkbox" checked={notifyOnInvite} onChange={(e) => setNotifyOnInvite(e.target.checked)} className="w-4 h-4 accent-primary rounded cursor-pointer"/>
                      </div>

                      <button type="submit" className="w-full py-2.5 bg-primary text-white text-xs font-bold rounded-xl hover:opacity-95 transition-opacity shadow-md shadow-primary/10 flex items-center justify-center gap-1.5">
              ...
              {/* ========================================= */}
              {subAdminActiveTab === "super-admin" && (<div className="flex flex-col gap-6">
                  <div className="flex-1 min-w-0 p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-xl space-y-6">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <button onClick={() => { setInviteModalRole("SuperAdmin"); setNewTeamMemberRole("SuperAdmin"); setShowInviteModal(true); }} className="shrink-0 flex items-center gap-1.5 px-3.5 py-2 bg-primary text-white text-[11px] font-bold rounded-xl hover:opacity-90 transition-opacity shadow-md shadow-primary/20">
                    ...
                    <div className="overflow-x-auto border border-slate-150 dark:border-slate-800/60 rounded-2xl">
                      <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-150 dark:divide-slate-800/50 text-xs">
                          {teamMembers.filter(m => m.role === "SuperAdmin").map((member) => (<tr key={member.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20 transition-colors">
                              ...
                              <td className="px-5 py-4">
                                <button onClick={() => toggleUserNotification(member.id)} className={`px-2.5 py-1 rounded-xl text-[10px] font-bold border transition-all flex items-center gap-1.5 ${userNotificationStatus[member.id] !== false
                        ...
                        navigator.clipboard.writeText(url);
                    }} className="text-slate-400 hover:text-primary" title="Copy link">
                              ...
                              <td className="px-5 py-4 text-right">
                                <button onClick={() => setNotificationModalTarget({ id: member.id, name: member.name, email: member.email, role: member.role, category: "Super Admin" })} className="p-1.5 rounded-lg text-slate-400 hover:text-primary hover:bg-primary/10 transition-colors" title="Send direct notification to Super Admin">
                                  <BellRing className="w-4 h-4"/>
                                </button>
                              </td>
                            </tr>))}
              ...
              {/* ========================================= */}
              {subAdminActiveTab === "sub-admins" && (<div className="flex flex-col gap-6">
                  ...
                  <div className="flex-1 min-w-0 p-4 sm:p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-xl space-y-5 sm:space-y-6">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                      ...
                      <button onClick={() => { setInviteModalRole("SubAdmin"); setNewTeamMemberRole("Operator"); setShowInviteModal(true); }} className="w-full sm:w-auto shrink-0 flex items-center justify-center gap-1.5 px-3.5 py-2 bg-primary text-white text-[11px] font-bold rounded-xl hover:opacity-90 transition-opacity shadow-md shadow-primary/20">
                        <Plus className="w-3.5 h-3.5"/>
                        Invite Sub Admin
                      </button>
                    </div>
                    ...
                    const sgId = member.sgId || `SG-A-${101 + idx}`;
                    const firstLetter = member.name ? member.name.trim().charAt(0).toUpperCase() : "A";
                    return (<div key={member.id} className="rounded-2xl border transition-all duration-300 bg-white dark:bg-[#0a0f1d] p-4 sm:p-5 flex flex-col justify-between space-y-4 border-slate-200 dark:border-slate-800/80 shadow-md hover:shadow-xl hover:-translate-y-1">
                            ...
                            <div className="flex items-center justify-between gap-2 shrink-0">
                              ...
                              <span className={`px-2.5 py-1 rounded-md text-[10px] font-extrabold ${member.role === "Developer"
                            ...
                            <div className="pt-3 border-t border-slate-150 dark:border-slate-800/80 flex flex-col space-y-2.5 shrink-0">
                              ...
                              <div className="flex items-center gap-2">
                                <button type="button" onClick={() => setSelectedSubAdminModal(member)} className="flex-1 py-1.5 rounded-md text-xs font-bold transition-all text-center bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-primary hover:text-white shadow-sm">
                                  View Options
                                </button>
                        ...
                        }} className="p-1.5 rounded-md bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white border border-red-500/20 transition-all shrink-0" title={`Delete ${member.name}`}>
              ...
              {showEmployeeModal && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowEmployeeModal(false)}>
                  <div className="absolute inset-0 bg-black/40 backdrop-blur-sm"/>
                  <div className="relative w-full max-w-sm p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-2xl space-y-6" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => setShowEmployeeModal(false)} className="absolute top-4 right-4 p-1.5 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                      <X className="w-4 h-4"/>
                    </button>
                    ...
                    <form onSubmit={(e) => { handleCreateEmployee(e); setShowEmployeeModal(false); }} className="space-y-4">
                      <div className="space-y-1.5">
                        ...
                        <input type="text" value={newEmpName} onChange={(e) => setNewEmpName(e.target.value)} placeholder="Enter employee name..." className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-primary" required autoFocus/>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input type="email" value={newEmpEmail} onChange={(e) => setNewEmpEmail(e.target.value)} placeholder="Enter employee email..." className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-primary" required/>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <select value={newEmpWork} onChange={(e) => setNewEmpWork(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <select value={newEmpAccess} onChange={(e) => setNewEmpAccess(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <select value={newEmpSubAdmin} onChange={(e) => setNewEmpSubAdmin(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary">
                          <option value="Unassigned">Unassigned</option>
                          {teamMembers.filter(tm => tm.role !== "SuperAdmin").map((tm) => (<option key={tm.id} value={tm.name}>
                              {tm.name} ({tm.role})
                            </option>))}
                        </select>
                      </div>
              ...
              {/* OPTION 3: EMPLOYEES TAB                   */}
              {/* ========================================= */}
              {subAdminActiveTab === "employees" && (<div className="flex flex-col gap-6">
                  ...
                  <div className="flex-1 min-w-0 p-4 sm:p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-xl space-y-5 sm:space-y-6">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                      ...
                      <button onClick={() => setShowEmployeeModal(true)} className="w-full sm:w-auto shrink-0 flex items-center justify-center gap-1.5 px-3.5 py-2 bg-primary text-white text-[11px] font-bold rounded-xl hover:opacity-90 transition-opacity shadow-md shadow-primary/20">
                        <Plus className="w-3.5 h-3.5"/>
                        Add New Employee
                      </button>
                    </div>
                    ...
                    return (<div key={emp.id} className="rounded-2xl border transition-all duration-300 bg-white dark:bg-[#0a0f1d] p-4 sm:p-5 flex flex-col justify-between space-y-4 border-slate-200 dark:border-slate-800/80 shadow-md hover:shadow-xl hover:-translate-y-1">
                            ...
                            <div className="pt-3 border-t border-slate-150 dark:border-slate-800/80 flex flex-col space-y-2.5 shrink-0">
                              ...
                              <div className="flex items-center gap-2">
                                <button type="button" onClick={() => setSelectedEmployeeModal(emp)} className="flex-1 py-1.5 rounded-md text-xs font-bold transition-all text-center bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-indigo-600 hover:text-white shadow-sm">
                                  View Options
                                </button>
                        ...
                        }} className="p-1.5 rounded-md bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white border border-red-500/20 transition-all shrink-0" title={`Delete ${emp.name}`}>
                                  <Trash className="w-3.5 h-3.5"/>
                                </button>
                              </div>
                            </div>
          ...
          {/* TAB 8: CONTROL FLAGS                    */}
          {/* ======================================= */}
          {activeView === "control-flags" && (<div className="space-y-6">
              <div className="p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-xl space-y-6">
                <div>
                  <h3 className="text-sm font-extrabold text-slate-900 dark:text-white font-display">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/40 border border-slate-150 dark:border-slate-800/60 flex items-center justify-between shadow-sm">
                    ...
                    <button onClick={() => setFeatureFlags({ ...featureFlags, aiVoiceEnabled: !featureFlags.aiVoiceEnabled })} className="focus:outline-none transition-colors">
                      {featureFlags.aiVoiceEnabled ? (<ToggleRight className="w-8 h-8 text-emerald-500"/>) : (<ToggleLeft className="w-8 h-8 text-slate-400"/>)}
                    </button>
                  </div>
                  ...
                  <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/40 border border-slate-150 dark:border-slate-800/60 flex items-center justify-between shadow-sm">
                    ...
                    <button onClick={() => setFeatureFlags({ ...featureFlags, whatsappEnabled: !featureFlags.whatsappEnabled })} className="focus:outline-none transition-colors">
                      {featureFlags.whatsappEnabled ? (<ToggleRight className="w-8 h-8 text-emerald-500"/>) : (<ToggleLeft className="w-8 h-8 text-slate-400"/>)}
                    </button>
                  </div>
                  ...
                  <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/40 border border-slate-150 dark:border-slate-800/60 flex items-center justify-between shadow-sm">
                    ...
                    <button onClick={() => setFeatureFlags({ ...featureFlags, autoGeoIpEnabled: !featureFlags.autoGeoIpEnabled })} className="focus:outline-none transition-colors">
                      {featureFlags.autoGeoIpEnabled ? (<ToggleRight className="w-8 h-8 text-emerald-500"/>) : (<ToggleLeft className="w-8 h-8 text-slate-400"/>)}
                    </button>
                  </div>
          ...
          {/* TAB 3: CLIENT DATA                      */}
          {/* ======================================= */}
          {activeView === "clients" && (<div className="p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 space-y-6 shadow-xl">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="relative w-full sm:w-48">
                    ...
                    <input type="text" placeholder="Search accounts..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-8 pr-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-primary"/>
                  </div>

                  {/* Plan filter */}
                  <select value={clientPlanFilter} onChange={(e) => setClientPlanFilter(e.target.value)} className="px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300 focus:outline-none focus:border-primary">
                  ...
                  {/* Status filter */}
                  <select value={clientStatusFilter} onChange={(e) => setClientStatusFilter(e.target.value)} className="px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-705 dark:text-slate-300 focus:outline-none focus:border-primary">
              ...
              <div className="overflow-x-auto border border-slate-100 dark:border-slate-800 rounded-2xl">
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 font-semibold text-slate-700 dark:text-slate-300">
                    {filteredClients.length > 0 ? (filteredClients.map((u) => (<tr key={u.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20 transition-colors">
                          <td className="py-3.5 px-4">
                            <div className="font-bold text-slate-900 dark:text-white">{u.name}</div>
                            <div className="text-[10px] text-slate-405 dark:text-slate-500">{u.email}</div>
                          </td>
                          <td className="py-3.5 px-4">
                            <span onClick={() => toggleUserRole(u.id)} className={`cursor-pointer px-2 py-0.5 rounded-full text-[9px] font-extrabold ${u.role === "SuperAdmin"
                    ? "bg-amber-500/10 text-amber-500 border border-amber-500/20"
                    : u.role === "Admin"
                        ? "bg-primary/10 text-primary border border-primary/20"
                        : "bg-slate-100 dark:bg-slate-800 text-slate-500"}`} title="Click to cycle role privilege level">
                              {u.role}
                            </span>
                          </td>
                          <td className="py-3.5 px-4">
                            <span onClick={() => changeUserPlan(u.id)} className="cursor-pointer text-slate-850 dark:text-slate-200 hover:text-primary underline decoration-dotted decoration-slate-400" title="Click to upgrade subscription plan tier">
                              {u.plan}
                            </span>
                          </td>
                          ...
                          <td className="py-3.5 px-4 text-right">
                            <button onClick={() => toggleUserStatus(u.id)} className={`px-2.5 py-1 rounded-lg border text-[10px] font-extrabold transition-colors ${u.status === "Active"
          ...
          {/* TAB 4: PAYMENT                          */}
          {/* ======================================= */}
          {activeView === "payments" && (<div className="space-y-6">
                ...
                <div className="overflow-x-auto border border-slate-100 dark:border-slate-800 rounded-2xl">
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 font-semibold text-slate-700 dark:text-slate-350">
                      {transactions.map((tx) => (<tr key={tx.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20 transition-colors">
                          ...
                          <td className="py-3 px-4">
                            <span className={`px-2 py-0.5 rounded text-[9px] font-extrabold ${tx.status === "Succeeded"
          ...
          {activeView === "products" && (<div className="space-y-6">
              ...
              <div className="p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-xl flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={handleOpenAddProduct} className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs flex items-center gap-2 shadow-lg shadow-emerald-600/20 transition-all active:scale-95">
                    <Plus className="w-4 h-4"/>
                    Add New Product
                  </button>
                  <a href="/products" target="_blank" rel="noreferrer" className="px-4 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-white font-bold rounded-xl text-xs flex items-center gap-2 transition-all">
              ...
              <div className="p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-xl space-y-6">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <select value={productCategoryFilter} onChange={(e) => setProductCategoryFilter(e.target.value)} className="px-3 py-2 bg-slate-100 dark:bg-slate-900 text-xs rounded-xl border border-slate-200 dark:border-slate-800 font-bold focus:outline-none focus:border-emerald-500">
                    ...
                    <div className="relative w-full sm:w-56">
                      ...
                      <input type="text" value={productSearchQuery} onChange={(e) => setProductSearchQuery(e.target.value)} placeholder="Search models..." className="w-full pl-9 pr-4 py-2 bg-slate-100 dark:bg-slate-900 text-xs rounded-xl border border-slate-200 dark:border-slate-800 focus:outline-none focus:border-emerald-500"/>
                    </div>

                    {/* Add Brochure Button */}
                    <button onClick={handleOpenAddBrochure} className="px-5 py-2.5 bg-gradient-to-r from-blue-600 via-indigo-600 to-emerald-500 hover:opacity-95 text-white text-xs font-extrabold rounded-full shadow-lg shadow-emerald-500/25 transition-all flex items-center gap-2 shrink-0 border border-white/10">
                ...
                .map((p) => (<div key={p.id} className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 flex flex-col justify-between space-y-4 hover:border-emerald-500/50 transition-all shadow-sm">
                        <div className="space-y-2">
                          {p.image ? (<div className="relative w-full h-36 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-800 mb-2 group/img flex items-center justify-center p-2">
                              <img src={p.image} alt={p.name} className="max-h-full max-w-full object-contain"/>
                              <button type="button" onClick={() => handleRemoveProductImage(p.id)} title="Remove Custom Image" className="absolute top-2 right-2 w-7 h-7 rounded-full bg-red-600 hover:bg-red-500 text-white flex items-center justify-center shadow-md transition-transform active:scale-95 z-10">
                                <Trash2 className="w-3.5 h-3.5"/>
                              </button>
                            </div>) : (<div className="relative w-full h-36 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-800 mb-2 flex items-center justify-center p-2">
                              <img src={PRODUCTS_DATA.find((d) => d.id === p.id)?.image || "/assets/brochures/brush_cutter_4sp_pr_page_1_img_1.png"} alt={p.name} onError={(e) => { e.currentTarget.src = "/assets/brochures/brush_cutter_4sp_pr_page_1_img_1.png"; }} className="max-h-full max-w-full object-contain"/>
                            </div>)}
                        ...
                        <div className="flex items-center gap-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                          <button onClick={() => handleOpenEditProduct(p)} className="flex-1 py-2 bg-slate-200 dark:bg-slate-800 hover:bg-emerald-600 hover:text-white text-slate-800 dark:text-white rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5">
                            <Edit className="w-3.5 h-3.5"/> Edit Model
                          </button>
                          <button onClick={() => handleDeleteProduct(p.id)} className="px-3 py-2 bg-red-500/10 hover:bg-red-500 hover:text-white text-red-500 rounded-xl text-xs font-bold transition-all" title="Delete Product">
                            <Trash2 className="w-3.5 h-3.5"/>
                          </button>
                        </div>
                      </div>))}
              ...
              {isAddProductModalOpen && (<div className="fixed inset-0 top-0 left-0 w-screen h-screen z-[99999] overflow-y-auto p-4 sm:p-6 bg-slate-950/80 backdrop-blur-sm flex items-start sm:items-center justify-center min-h-screen py-8">
                  <div className="relative w-full max-w-2xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-2xl space-y-4 max-h-[85vh] overflow-y-auto my-auto">
                    ...
                    <div className="sticky top-0 z-20 bg-white/95 dark:bg-[#0a0f1d]/95 backdrop-blur-md pb-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                      ...
                      <button onClick={() => setIsAddProductModalOpen(false)} className="p-1.5 rounded-full text-slate-400 hover:text-white hover:bg-slate-800">
                        <X className="w-5 h-5"/>
                      </button>
                    </div>
                    ...
                    <form onSubmit={handleSaveProductForm} className="space-y-4 text-xs">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input type="text" required placeholder="e.g. George Maijo BC 520 2SP" value={prodForm.name} onChange={(e) => setProdForm({ ...prodForm, name: e.target.value })} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                        </div>

                        <div>
                          <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">Category *</label>
                          <select value={prodForm.category} onChange={(e) => setProdForm({ ...prodForm, category: e.target.value })} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold focus:outline-none focus:border-emerald-500">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                          ...
                          <input type="text" value={prodForm.brand} onChange={(e) => setProdForm({ ...prodForm, brand: e.target.value })} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                        </div>

                        <div>
                          <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">Engine Specs</label>
                          <input type="text" placeholder="e.g. 42.7cc 2-Stroke" value={prodForm.engine} onChange={(e) => setProdForm({ ...prodForm, engine: e.target.value })} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                        </div>

                        <div>
                          <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">Power Output</label>
                          <input type="text" placeholder="e.g. 1.25 kW @ 7000 RPM" value={prodForm.power} onChange={(e) => setProdForm({ ...prodForm, power: e.target.value })} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                          ...
                          <input type="text" placeholder="e.g. 42.7 cc" value={prodForm.displacement} onChange={(e) => setProdForm({ ...prodForm, displacement: e.target.value })} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                        </div>

                        <div>
                          <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">Weight</label>
                          <input type="text" placeholder="e.g. 7.5 kg" value={prodForm.weight} onChange={(e) => setProdForm({ ...prodForm, weight: e.target.value })} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                        </div>

                        <div>
                          <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">Cutting / Tilling Width</label>
                          <input type="text" placeholder="e.g. 430 mm" value={prodForm.cuttingWidth} onChange={(e) => setProdForm({ ...prodForm, cuttingWidth: e.target.value })} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                        </div>
                      </div>
                      ...
                      <div className="p-3 bg-slate-50 dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-2">
                        ...
                        <div className="flex items-center gap-3">
                          <input type="file" accept="image/*" onChange={handleProductImageUpload} className="text-xs text-slate-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-emerald-600 file:text-white hover:file:bg-emerald-500 cursor-pointer"/>
                          {prodForm.image && (<button type="button" onClick={() => setProdForm({ ...prodForm, image: "" })} className="text-xs text-red-500 font-bold hover:underline">
                              Remove Image
                            </button>)}
                        </div>
                        {prodForm.image && (<div className="mt-2 w-24 h-24 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700">
                      ...
                      <div>
                        ...
                        <input type="text" required placeholder="Brief 1-line overview of the product..." value={prodForm.shortDesc} onChange={(e) => setProdForm({ ...prodForm, shortDesc: e.target.value })} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                      </div>

                      <div>
                        <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">Full Detailed Description</label>
                        <textarea rows={3} placeholder="Comprehensive features, application fields, and performance overview..." value={prodForm.fullDesc} onChange={(e) => setProdForm({ ...prodForm, fullDesc: e.target.value })} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                      </div>

                      <div>
                        <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">Key Highlights (Comma Separated)</label>
                        <input type="text" placeholder="e.g. Powerful Engine, 430mm Cutting Width, Low Vibration, Lightweight" value={prodForm.highlightsText} onChange={(e) => setProdForm({ ...prodForm, highlightsText: e.target.value })} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs focus:outline-none focus:border-emerald-500"/>
                      </div>
                      ...
                      <div className="sticky bottom-0 z-20 bg-white/95 dark:bg-[#0a0f1d]/95 backdrop-blur-md pt-3 pb-1 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
                        <button type="button" onClick={() => setIsAddProductModalOpen(false)} className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold rounded-xl text-xs hover:bg-slate-200 transition-colors">
                          Cancel
                        </button>
                        <button type="submit" className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs shadow-md shadow-emerald-600/20 active:scale-95 transition-all">
          ...
          {/* ======================================= */}
          {/* TAB 5: SERVICES                         */}
          {activeView === "services" && (<div className="space-y-6">
              ...
              <div className="p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-xl flex items-center justify-between">
                ...
                <button onClick={() => setShowAddServiceForm(!showAddServiceForm)} className="px-3.5 py-2 bg-primary text-white text-xs font-bold rounded-xl hover:opacity-95 transition-opacity shadow-md shadow-primary/10 flex items-center gap-1.5 shrink-0">
                  <Plus className="w-4 h-4"/>
              ...
              {showAddServiceForm && (<motion.form initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleAddService} className="p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-xl space-y-4 max-w-xl">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <input type="text" required value={newServiceName} onChange={(e) => setNewServiceName(e.target.value)} placeholder="e.g. SMTP Mail Dispatcher" className="w-full px-3 py-2 rounded-xl bg-slate-5-0 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Initial Status</label>
                      <select value={newServiceStatus} onChange={(e) => setNewServiceStatus(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary">
                        <option value="Active">Active</option>
                    ...
                    <div className="space-y-1.5 sm:col-span-2">
                      ...
                      <input type="text" value={newServiceDesc} onChange={(e) => setNewServiceDesc(e.target.value)} placeholder="Short description of what this microservice does..." className="w-full px-3 py-2 rounded-xl bg-slate-5-0 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-505 uppercase tracking-wider">Feature 1</label>
                      <input type="text" required value={newServiceFeature1} onChange={(e) => setNewServiceFeature1(e.target.value)} placeholder="e.g. Official Meta API Broadcasts" className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-505 uppercase tracking-wider">Feature 2</label>
                      <input type="text" required value={newServiceFeature2} onChange={(e) => setNewServiceFeature2(e.target.value)} placeholder="e.g. Unified Shared Agent Inbox" className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-slate-150 dark:border-slate-800/80 pt-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <div className="space-y-1">
                        ...
                        <input type="number" required value={newPriceMonthlyINR} onChange={(e) => {
                ...
                }} placeholder="e.g. 1999" className="w-full px-2.5 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                      </div>
                      <div className="space-y-1">
                        <label className="text-[9px] font-bold text-slate-550 uppercase tracking-wider block">Monthly (USD)</label>
                        <input type="number" required value={newPriceMonthlyUSD} onChange={(e) => setNewPriceMonthlyUSD(e.target.value)} placeholder="e.g. 29" className="w-full px-2.5 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                      </div>
                      <div className="space-y-1">
                        <label className="text-[9px] font-bold text-slate-550 uppercase tracking-wider block">Yearly (INR)</label>
                        <input type="number" required value={newPriceYearlyINR} onChange={(e) => {
                    const val = e.target.value;
                ...
                }} placeholder="e.g. 19999" className="w-full px-2.5 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                      </div>
                      <div className="space-y-1">
                        <label className="text-[9px] font-bold text-slate-550 uppercase tracking-wider block">Yearly (USD)</label>
                        <input type="number" required value={newPriceYearlyUSD} onChange={(e) => setNewPriceYearlyUSD(e.target.value)} placeholder="e.g. 299" className="w-full px-2.5 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                      </div>
                    </div>
                  </div>

                  <div className="flex justify-end gap-2 pt-2">
                    <button type="button" onClick={() => setShowAddServiceForm(false)} className="px-4 py-2 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-xl transition-colors">
                      Cancel
                    </button>
                    <button type="submit" className="px-4 py-2 bg-primary text-white text-xs font-bold rounded-xl hover:opacity-90 transition-opacity">
                      Save Service
                    </button>
                  </div>
                </motion.form>)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {services.map((srv) => (<div key={srv.id} onClick={() => {
                    if (editingServiceId !== srv.id) {
                        handleStartEdit(srv);
                    }
                }} className={`p-5 rounded-2xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-md flex flex-col justify-between gap-4 relative group/card transition-all duration-300 ${editingServiceId === srv.id
                ...
                /* Editing View */
                <form onSubmit={handleSaveEdit} onClick={(e) => e.stopPropagation()} className="space-y-3.5">
                        <div className="text-xs font-bold text-slate-900 dark:text-white">Editing {srv.name}</div>
                        
                        <div className="space-y-2 text-xs">
                          <input type="text" required value={editServiceName} onChange={(e) => setEditServiceName(e.target.value)} placeholder="Service Name" className="w-full px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none"/>
                          <input type="text" value={editServiceDesc} onChange={(e) => setEditServiceDesc(e.target.value)} placeholder="Description" className="w-full px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-805 text-xs text-slate-900 dark:text-white focus:outline-none"/>
                          <div className="grid grid-cols-2 gap-2">
                            <div>
                              <label className="block text-[8px] text-slate-400 uppercase font-bold">Feature 1</label>
                              <input type="text" required value={editServiceFeature1} onChange={(e) => setEditServiceFeature1(e.target.value)} className="w-full px-2 py-1 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none"/>
                            </div>
                            <div>
                              <label className="block text-[8px] text-slate-400 uppercase font-bold">Feature 2</label>
                              <input type="text" required value={editServiceFeature2} onChange={(e) => setEditServiceFeature2(e.target.value)} className="w-full px-2 py-1 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none"/>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-slate-100 dark:border-slate-800/80">
                            <div>
                              ...
                              <input type="number" required value={editPriceMonthlyINR} onChange={(e) => {
                        const val = e.target.value;
                        ...
                        if (!isNaN(parsed)) {
                            ...
                            <div>
                              ...
                              <input type="number" required value={editPriceMonthlyUSD} onChange={(e) => setEditPriceMonthlyUSD(e.target.value)} className="w-full px-2 py-1 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white focus:outline-none"/>
                            </div>
                            <div>
                              <label className="block text-[8px] text-slate-400 uppercase font-bold">Yearly (INR)</label>
                              <input type="number" required value={editPriceYearlyINR} onChange={(e) => {
                        const val = e.target.value;
                        ...
                        if (!isNaN(parsed)) {
                            ...
                            <div>
                              ...
                              <input type="number" required value={editPriceYearlyUSD} onChange={(e) => setEditPriceYearlyUSD(e.target.value)} className="w-full px-2 py-1 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-805 text-xs text-slate-900 dark:text-white focus:outline-none"/>
                            </div>
                          </div>
                        </div>

                        <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800/80">
                          <button type="button" onClick={() => setEditingServiceId(null)} className="px-2.5 py-1 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 text-[10px] font-bold rounded-lg transition-colors">
                            Cancel
                          </button>
                          <button type="submit" className="px-2.5 py-1 bg-primary text-white text-[10px] font-bold rounded-lg hover:opacity-90 transition-opacity">
                            Save Changes
                          </button>
                        </div>
                      </form>) : (
                ...
                <>
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <div className="text-xs font-bold text-slate-900 dark:text-white flex items-center gap-2">
                              {srv.name}
                              <span className={`w-2 h-2 rounded-full ${srv.status === "Active"
                        ? "bg-emerald-500"
                        : srv.status === "Inactive"
                            ? "bg-red-500"
                        ...
                        e.stopPropagation();
                        handleStartEdit(srv);
                    }} className="p-1 rounded-lg text-slate-400 hover:text-primary hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" title="Edit Service">
                                <Edit className="w-3.5 h-3.5"/>
                              </button>
                              <button onClick={(e) => {
                        e.stopPropagation();
                        handleDeleteService(srv.id);
                    }} className="p-1 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-500/10 transition-colors" title="Delete Service">
                                <Trash className="w-3.5 h-3.5"/>
                              </button>
                            </div>
                    ...
                    }} className={`px-3 py-1 rounded-xl text-[9px] font-extrabold border transition-colors shrink-0 ${srv.status === "Active"
                        ...
                        <div className="pt-3 border-t border-slate-100 dark:border-slate-800/80 space-y-1.5 text-[10px]">
                          {(srv.features && srv.features.length > 0 ? srv.features : getDefaultFeatures(srv.name, srv.id)).map((feature, idx) => (<div key={idx} className="flex items-center gap-2">
          ...
          {/* TAB 6: SETTINGS                         */}
          {/* ======================================= */}
          {activeView === "settings" && (<div className="p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 space-y-6 shadow-xl">
              <div>
                <h3 className="text-sm font-extrabold text-slate-900 dark:text-white font-display">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80">
                  ...
                  <button onClick={() => setFeatureFlags(prev => ({ ...prev, debugLogsEnabled: !prev.debugLogsEnabled }))} className={`p-1 transition-colors ${featureFlags.debugLogsEnabled ? "text-emerald-500" : "text-slate-400"}`}>
                ...
                <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80">
                  ...
                  <button onClick={() => setFeatureFlags(prev => ({ ...prev, maintenanceMode: !prev.maintenanceMode }))} className={`p-1 transition-colors ${featureFlags.maintenanceMode ? "text-emerald-500" : "text-slate-400"}`}>
              ...
              <div className="p-4 rounded-2xl bg-red-500/5 border border-red-500/20 space-y-4">
                ...
                <div className="flex items-center gap-2">
                  <input type="text" readOnly value={systemSecretKey} className="flex-1 px-3 py-2 rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-250 dark:border-slate-850 font-mono text-[10px] text-slate-800 dark:text-slate-350 focus:outline-none"/>
                  <button onClick={rotateSecretKey} className="px-4 py-2 bg-red-600 dark:bg-red-500 text-white text-xs font-bold rounded-xl hover:bg-red-700 transition-colors flex items-center gap-2 shrink-0">
          ...
          {/* TAB 7: MY PROFILE                       */}
          {/* ======================================= */}
          {activeView === "profile" && (<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800/80 shadow-xl space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="text" value={profileName} onChange={(e) => setProfileName(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="email" value={profileEmail} onChange={(e) => setProfileEmail(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-850 text-xs font-semibold text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="password" value={profilePasscode} onChange={(e) => setProfilePasscode(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="text" value={profileAuthKey} onChange={(e) => setProfileAuthKey(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="text" value={profileAdminLevel} onChange={(e) => setProfileAdminLevel(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="text" value={profileCountry} onChange={(e) => setProfileCountry(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-900 dark:text-white focus:outline-none focus:border-primary"/>
                  </div>
                </div>

                <div className="flex justify-end pt-3">
                  <button onClick={handleSaveProfile} className="px-4 py-2 bg-primary text-white text-xs font-bold rounded-xl hover:opacity-90 transition-opacity">
      ...
      {invitationSuccessModal && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-md p-6 rounded-3xl bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-slate-800 shadow-2xl space-y-6 relative">
            <div className="flex items-center gap-3">
              ...
              <div className="pt-2.5 border-t border-slate-200 dark:border-slate-800 space-y-2">
                <div className="space-y-1">
                  ...
                  <div className="flex items-center justify-between p-2 rounded-xl bg-slate-100 dark:bg-slate-950 font-mono text-[11px] font-bold text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800">
                    ...
                    <button onClick={() => navigator.clipboard.writeText(invitationSuccessModal.tempPass)} className="text-[10px] text-primary hover:underline font-sans">
                      Copy
                    </button>
                  </div>
                </div>
                ...
                <div className="space-y-1 pt-1.5">
                  ...
                  <div className="flex items-center justify-between p-2 rounded-xl bg-slate-100 dark:bg-slate-950 font-mono text-[10px] font-bold text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800 overflow-hidden">
                    ...
                    <button onClick={() => navigator.clipboard.writeText(invitationSuccessModal.loginUrl)} className="text-[10px] text-primary hover:underline font-sans shrink-0">
                      Copy Link
                    </button>
                  </div>
                </div>
            ...
            <button onClick={() => setInvitationSuccessModal(null)} className="w-full py-2.5 bg-primary text-white text-xs font-bold rounded-xl hover:opacity-95 transition-opacity">
              Done & Close
            </button>
          </motion.div>
        </div>)}
      ...
      {selectedSubAdminModal && (<div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto animate-fadeIn">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl max-w-2xl w-full p-6 space-y-6 relative my-8">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
              ...
              <button onClick={() => setSelectedSubAdminModal(null)} className="p-2 rounded-xl text-slate-400 hover:text-slate-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                <X className="w-5 h-5"/>
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1 bg-slate-50 dark:bg-slate-950 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <input type="text" value={selectedSubAdminModal.sgId || ""} onChange={(e) => {
                const val = e.target.value;
                setSelectedSubAdminModal({ ...selectedSubAdminModal, sgId: val });
                handleUpdateTeamMember(selectedSubAdminModal.id, "sgId", val);
            }} className="w-full px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-mono font-extrabold text-primary focus:outline-none focus:border-primary" placeholder="SG-A-101..."/>
              </div>
              ...
              <div className="space-y-1 bg-slate-50 dark:bg-slate-950 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <input type="text" value={selectedSubAdminModal.name || ""} onChange={(e) => {
                const val = e.target.value;
                setSelectedSubAdminModal({ ...selectedSubAdminModal, name: val });
                handleUpdateTeamMember(selectedSubAdminModal.id, "name", val);
            }} className="w-full px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:border-primary" placeholder="Edit Name..."/>
              </div>
              ...
              <div className="space-y-1 bg-slate-50 dark:bg-slate-950 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <select value={selectedSubAdminModal.role} onChange={(e) => {
                const val = e.target.value;
              ...
              <div className="space-y-1 bg-slate-50 dark:bg-slate-950 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 col-span-1 sm:col-span-2">
                ...
                <select value={selectedSubAdminModal.permissions} onChange={(e) => {
                const val = e.target.value;
      ...
      {selectedEmployeeModal && (<div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto animate-fadeIn">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl max-w-2xl w-full p-6 space-y-6 relative my-8">
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
              <div className="flex items-center gap-3">
              ...
              <button onClick={() => setSelectedEmployeeModal(null)} className="p-2 rounded-xl text-slate-400 hover:text-slate-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                <X className="w-5 h-5"/>
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1 bg-slate-50 dark:bg-slate-950 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <input type="text" value={selectedEmployeeModal.name || ""} onChange={(e) => {
                const val = e.target.value;
              ...
              <div className="space-y-1 bg-slate-50 dark:bg-slate-950 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <input type="text" value={selectedEmployeeModal.email || ""} onChange={(e) => {
                const val = e.target.value;
                setSelectedEmployeeModal({ ...selectedEmployeeModal, email: val });
                handleUpdateEmployee(selectedEmployeeModal.id, "email", val);
            }} className="w-full px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-mono font-bold text-slate-800 dark:text-slate-200 focus:outline-none focus:border-primary" placeholder="Edit Email..."/>
              </div>
              ...
              <div className="space-y-1 bg-slate-50 dark:bg-slate-950 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <select value={selectedEmployeeModal.assignedSubAdmin} onChange={(e) => {
                const val = e.target.value;
                ...
                handleUpdateEmployee(selectedEmployeeModal.id, "assignedSubAdmin", val);
                  ...
                  <option value="Unassigned">Unassigned</option>
                  {teamMembers.filter(tm => tm.role !== "SuperAdmin").map(tm => (<option key={tm.id} value={tm.name}>{tm.name} ({tm.role})</option>))}
                </select>
              </div>
              ...
              <div className="space-y-1 bg-slate-50 dark:bg-slate-950 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <input type="text" value={selectedEmployeeModal.work || ""} onChange={(e) => {
                const val = e.target.value;
                setSelectedEmployeeModal({ ...selectedEmployeeModal, work: val });
                handleUpdateEmployee(selectedEmployeeModal.id, "work", val);
            }} className="w-full px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-bold text-slate-800 dark:text-slate-200 focus:outline-none focus:border-primary" placeholder="Task description..."/>
              </div>
              ...
              <div className="space-y-1 bg-slate-50 dark:bg-slate-950 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 col-span-1 sm:col-span-2">
                ...
                <select value={selectedEmployeeModal.access} onChange={(e) => {
                const val = e.target.value;
      ...
      {/* ======================================================== */}
      {isBrochureModalOpen && (<div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn overflow-y-auto">
          <div className="relative w-full max-w-2xl max-h-[90vh] flex flex-col bg-card border border-border rounded-3xl shadow-2xl my-auto text-left animate-scaleUp overflow-hidden text-foreground">
            ...
            <div className="flex flex-col gap-4 p-6 border-b border-border bg-card shrink-0">
              <div className="flex items-start justify-between gap-4">
                ...
                <button type="button" onClick={() => setIsBrochureModalOpen(false)} className="p-2 rounded-xl text-muted-foreground hover:text-foreground hover:bg-muted transition-colors shrink-0">
                  <X className="w-5 h-5"/>
                </button>
              </div>
              ...
              {brochureStep === 1 && (<div className="space-y-4 animate-fadeIn">
                  ...
                  <div className="space-y-4 p-6 rounded-2xl border-2 border-dashed border-blue-500/40 bg-blue-500/5 text-center">
                    ...
                    await handleAnalyzePdfBrochure(brochureData.pdfFileName || "Brush_Cutter_4SP_PR_Brochure.pdf");
                      ...
                      <input type="text" value={brochureData.pdfFileName} onChange={(e) => setBrochureData(prev => ({ ...prev, pdfFileName: e.target.value }))} placeholder="e.g. Brush_Cutter_4SP_PR_Brochure.pdf" className="w-full p-2.5 rounded-xl border border-border bg-background text-xs font-mono font-bold focus:outline-none focus:border-primary text-foreground"/>
                    </div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-border gap-3">
                    <button type="button" onClick={() => setIsBrochureModalOpen(false)} className="px-5 py-2.5 border border-border rounded-xl hover:bg-muted text-muted-foreground text-xs font-extrabold transition-all">
                      Cancel
                    </button>
                    <button type="button" disabled={isAnalyzingPdf} onClick={async () => {
                    await handleAnalyzePdfBrochure(brochureData.pdfFileName || "Brush_Cutter_4SP_PR_Brochure.pdf");
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-2xl border border-blue-500/30 bg-blue-500/5 dark:bg-slate-900/60">
                    ...
                    <div className="sm:col-span-2 space-y-3">
                      <div className="space-y-1">
                        ...
                        <input type="text" required value={brochureData.name} onChange={(e) => setBrochureData(prev => ({ ...prev, name: e.target.value }))} placeholder="e.g. George Maijo Brush Cutter 4SP PR" className="w-full p-2.5 rounded-xl border border-border bg-background text-xs font-bold focus:outline-none focus:border-primary text-foreground"/>
                      </div>

                      <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                          <label className="font-extrabold text-foreground text-[10px] uppercase tracking-wider">Category</label>
                          <select value={brochureData.category} onChange={(e) => setBrochureData(prev => ({ ...prev, category: e.target.value }))} className="w-full p-2 rounded-xl border border-border bg-background text-xs font-bold focus:outline-none focus:border-primary text-foreground">
                            <option value="Brush Cutter">Brush Cutter</option>
                            <option value="Power Weeder">Power Weeder</option>
                            <option value="Power Tiller">Power Tiller</option>
                            <option value="Combine Harvester">Combine Harvester</option>
                      ...
                      <div className="space-y-1">
                        ...
                        <textarea rows={2} required value={brochureData.shortDesc} onChange={(e) => setBrochureData(prev => ({ ...prev, shortDesc: e.target.value }))} className="w-full p-2 rounded-xl border border-border bg-background text-[11px] leading-relaxed focus:outline-none focus:border-primary font-medium text-foreground"/>
                      </div>
                    </div>
