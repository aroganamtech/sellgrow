...
import { Home, Zap, Tag, Info, PhoneCall, ArrowRight, Sparkles, } from "lucide-react";
...
export default function HomeEditSelectionPage() {
    ...
    if (isLoading || !isAuthenticated) {
        return (<div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center text-slate-500 dark:text-slate-400 text-sm">
    ...
    return (<div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans transition-colors duration-200">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-8 flex flex-col justify-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-4">
          {sections.map((sec) => {
            const Icon = sec.icon;
            return (<Link key={sec.id} href={sec.href}>
                <motion.div whileHover={{ scale: 1.02, y: -4 }} whileTap={{ scale: 0.98 }} className="group h-full p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-primary/50 transition-all duration-300 shadow-xl flex flex-col justify-between space-y-5 relative overflow-hidden">
