...
import { Calendar, Clock, User, Mail, Phone, PhoneCall, Trash2, CheckCircle, Search, ArrowLeft, Sparkles, ClipboardList } from "lucide-react";
...
export default function ServicePersonPortal() {
    ...
    const saveBookingsState = (updatedList) => {
    ...
    return (<div className="min-h-screen bg-slate-50 dark:bg-[#070b14] text-foreground transition-all duration-300 pb-12">
      {/* Top Sticky Header */}
      <header className="sticky top-0 z-40 w-full border-b border-border bg-white/80 dark:bg-[#0c1220]/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/" className="p-2 rounded-xl border hover:bg-slate-100 dark:hover:bg-white/5 text-muted-foreground hover:text-foreground transition-all flex items-center gap-1.5 text-xs font-semibold">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center p-4 border border-border bg-white dark:bg-[#0c1220] rounded-2xl shadow-sm">
        ...
        ].map(tab => (<button key={tab.id} onClick={() => setFilter(tab.id)} className={`flex-grow md:flex-grow-0 px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${filter === tab.id
          ...
          <div className="relative w-full md:w-72">
            <input type="text" placeholder="Search by customer name or product..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2 text-xs rounded-xl border border-border bg-slate-50 dark:bg-black/20 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary"/>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="lg:col-span-2 space-y-4">
            {filteredBookings.length === 0 ? (<div className="p-12 border border-dashed border-border bg-white dark:bg-[#0c1220] rounded-2xl text-center space-y-3">
                ...
                {filteredBookings.map((booking) => (<div key={booking.id} onClick={() => handleSelectBooking(booking)} className={`p-5 rounded-2xl border transition-all text-left space-y-4 cursor-pointer hover:shadow-md ${selectedBooking?.id === booking.id
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className={`text-[10px] font-bold uppercase px-2.5 py-0.5 rounded-full ${booking.status === "completed"
                ...
                }} className="p-2 rounded-xl text-muted-foreground hover:text-red-500 hover:bg-red-500/10 border border-transparent hover:border-red-500/20 transition-all shrink-0" aria-label="Delete booking">
          ...
          <div className="lg:col-span-1">
            {selectedBooking ? (<div className="p-6 rounded-2xl border border-border bg-white dark:bg-[#0c1220] shadow-sm space-y-6 text-left animate-fade-in">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => updateBookingStatus(selectedBooking.id, "completed")} className={`flex-grow py-2 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 border transition-all ${selectedBooking.status === "completed"
                ? "bg-emerald-600 border-emerald-600 text-white"
                : "border-border hover:bg-slate-100 dark:hover:bg-white/5 text-foreground"}`}>
                      <CheckCircle className="w-3.5 h-3.5"/> Complete
                    </button>
                    <button onClick={() => updateBookingStatus(selectedBooking.id, "cancelled")} className={`flex-grow py-2 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 border transition-all ${selectedBooking.status === "cancelled"
                ...
                {selectedBooking.status === "pending" && (<div className="p-4 rounded-xl border border-primary/20 bg-primary/5 space-y-3">
                    ...
                    <button onClick={() => alert(`Launching live simulation for ${selectedBooking.name} on ${selectedBooking.solutionTitle}...`)} className="w-full py-2 bg-primary hover:opacity-90 text-white rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5">
                ...
                <form onSubmit={handleSaveNotes} className="space-y-2.5 pt-2">
                  ...
                  <textarea id="appointment-notes" rows={4} value={noteInput} onChange={(e) => setNoteInput(e.target.value)} placeholder="Enter customer requests, specific configurations, or follow-up timelines..." className="w-full p-3 text-xs rounded-xl border border-border bg-slate-50 dark:bg-black/20 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary"/>
                  <button type="submit" className="w-full py-2 border border-primary text-primary hover:bg-primary/5 rounded-xl text-xs font-bold transition-all">
                    Save Notes Update
                  </button>
                </form>

              </div>) : (<div className="p-6 rounded-2xl border border-border bg-white dark:bg-[#0c1220] shadow-sm text-center py-12 space-y-3">
                <ClipboardList className="w-10 h-10 text-muted mx-auto"/>
