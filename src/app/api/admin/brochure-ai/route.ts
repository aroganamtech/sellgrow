  ...
  try {
    ...
    if (action === "self-train") {
      ...
      return NextResponse.json(
        { success: true, message: `🧠 Brochure AI Model trained on "${fileName || productName}" successfully!` },
        { status: 200 }
      );
    }
    ...
    if (!fileName && !productName) {
      return NextResponse.json(
        { error: "fileName or productName is required" },
        { status: 400 }
      );
    }

    const analysis = await BrochureAiPipelineService.processBrochureDocument(
      fileName || "brochure.pdf",
      productName,
      brand || "George Maijo Agri"
    );
    ...
    return NextResponse.json(
      { error: err.message || "Failed to process brochure PDF" },
      { status: 500 }
    );
  }
}
