...
import { formatUserResponse } from '@/lib/models/User';
export async function POST(request) {
    try {
        ...
        if (!userFirstName || !email || !password || !businessName) {
            return NextResponse.json({ status: 'error', message: 'Please fill in all required fields (First Name, Email, Password, Business Name).' }, { status: 400 });
        }
        ...
        if (existingUser) {
            return NextResponse.json({ status: 'error', message: 'An account with this email address already exists. Please login.' }, { status: 409 });
        }
        // Create user object
        const newUser = {
            name: userFirstName,
        ...
        await usersCol.updateOne({ email: normalizedEmail }, { $set: newUser }, { upsert: true });
        ...
        return NextResponse.json({ status: 'error', message: error.message || 'Internal server error during registration.' }, { status: 500 });
    }
}
