...
export async function POST(request) {
    try {
        ...
        if (!user) {
            return NextResponse.json({ status: 'error', message: 'Invalid email or password. Please register an account first.' }, { status: 401 });
        }
        ...
        const now = new Date();
        await usersCol.updateOne({ email: normalizedEmail }, { $set: { lastLogin: now, updatedAt: now } });
        ...
        return NextResponse.json({ status: 'error', message: error.message || 'Internal server error during login.' }, { status: 500 });
    }
}
