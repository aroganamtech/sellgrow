...
export class ProductPdfIntelligenceModel {
    ...
    static selfTrainedMemory = new Map();
    /**
     ...
     */
    static normalizeSpecKey(rawKey) {
        const k = rawKey.toLowerCase().trim();
        if (/engine|motor|powerplant/i.test(k) && !/displacement|power|capacity/i.test(k))
            return "Engine Model";
        if (/power|hp|kw|output/i.test(k) && !/fuel|tank/i.test(k))
            return "Max Power Output";
        if (/displacement|cc/i.test(k))
            return "Engine Displacement";
        if (/width|tilling|working|cultivation/i.test(k))
            return "Working / Tilling Width";
        if (/depth|penetration|soil/i.test(k))
            return "Tilling Depth";
        if (/gear|transmission|speed|drive/i.test(k))
            return "Transmission Type";
        if (/fuel|tank|petrol|diesel/i.test(k) && !/consumption/i.test(k))
            return "Fuel Tank Capacity";
        if (/consumption|mileage|rate/i.test(k))
            return "Fuel Consumption Rate";
        if (/weight|mass|kg/i.test(k))
            return "Net Dry Weight";
        if (/tines|blade|blade count/i.test(k))
            return "Rotary Blade Tines";
        if (/starter|starting|recoil/i.test(k))
            return "Starting Mechanism";
        if (/tire|wheel|rubber/i.test(k))
            return "Tire / Wheel Size";
        return rawKey.trim();
    }
    /**
