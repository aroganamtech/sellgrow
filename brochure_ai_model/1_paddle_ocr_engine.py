...
class PaddleOCREngine:
    ...
    def process_image(self, image_path: str) -> List[Dict[str, Any]]:
        ...
        if self.ocr is None:
            # Fallback simulated OCR output for non-Paddle environments
        ...
        if results and results[0]:
            for line in results[0]:
                box, (text, score) = line
                ocr_blocks.append({
                    "box": box, # [[x1,y1], [x2,y1], [x2,y2], [x1,y2]]
                    "text": text.strip(),
                    "score": float(score)
                })
        return ocr_blocks
    ...
    def process_pdf(self, pdf_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Fallback simulation if pdf2image or poppler is unavailable
            pages_result.append({
                "page_index": 0,
                "width": 1200,
                "height": 1600,
...
if __name__ == "__main__":
    ...
    print(f"✅ Stage 1 PaddleOCR extraction completed successfully! Saved to {args.output}")
