...
export interface SolutionItemModalData {
  id: "voice" | "whatsapp" | "app" | "website";
  title: string;
  shortDesc: string;
  longDesc: string;
...
interface SolutionModalProps {
  isOpen: boolean;
  onClose: () => void;
  solution: SolutionItemModalData | null;
...
export default function SolutionModal({
  ...
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState<"overview" | "demo" | "specs" | "code">("demo");

  // Voice simulator state
  const [isCalling, setIsCalling] = useState(false);
  const [callDuration, setCallDuration] = useState(0);
  const [isMuted, setIsMuted] = useState(false);
  const [voiceLang, setVoiceLang] = useState<"en" | "ta" | "hi" | "ar">("ta");
  const [customInput, setCustomInput] = useState("");
  const [isListening, setIsListening] = useState(false);
  const [isSpeaking, setIsSpeaking] = useState(false);
  const [transcript, setTranscript] = useState<Array<{ sender: "user" | "ai"; text: string; time: string }>>([]);
  ...
  const [appTab, setAppTab] = useState<"home" | "products" | "notification">("home");

  // TTS Helper using Web Speech API
  const speakText = (text: string, lang: "en" | "ta" | "hi" | "ar") => {
  ...
  const sendVoiceQuery = (textToSend?: string) => {
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
        ...
        <motion.div
          ...
          className="relative w-full max-w-4xl bg-white dark:bg-[#0b1120] border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden z-10 my-8 max-h-[90vh] flex flex-col"
        >
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
            ...
            <button
              onClick={onClose}
              className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-white rounded-full hover:bg-slate-200/50 dark:hover:bg-slate-800 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="p-6 overflow-y-auto flex-1 space-y-6">
            ...
            {activeTab === "demo" && (
              <div className="space-y-6">
                ...
                {solution.id === "voice" && (
                  <div className="bg-slate-950 text-white rounded-2xl p-6 border border-slate-800 space-y-6 shadow-inner">
                      ...
                      <div className="flex items-center gap-1.5 bg-slate-900 p-1 rounded-xl border border-slate-800">
                        ...
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${
                            voiceLang === "ta" ? "bg-sky-500 text-white" : "text-slate-400 hover:text-white"
                          }`}
                        >
                          🇮🇳 தமிழ் (Tamil)
                        </button>
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${
                            voiceLang === "en" ? "bg-sky-500 text-white" : "text-slate-400 hover:text-white"
                          }`}
                        >
                          🇺🇸 English
                        </button>
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${
                            voiceLang === "hi" ? "bg-sky-500 text-white" : "text-slate-400 hover:text-white"
                          }`}
                        >
                          🇮🇳 हिंदी
                        </button>
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${
                            voiceLang === "ar" ? "bg-sky-500 text-white" : "text-slate-400 hover:text-white"
                          }`}
                        >
                          🇸🇦 العربية
                        </button>
                    ...
                    <div className="flex flex-col items-center justify-center py-6 bg-slate-900/60 rounded-xl border border-slate-800/80 relative overflow-hidden">
                      ...
                      {(isCalling || isSpeaking) && (
                        <div className="flex items-center gap-1.5 h-8 my-2">
                          {[40, 70, 30, 90, 50, 80, 40, 100, 60, 30, 80, 40].map((h, i) => (
                            <motion.div
                              key={i}
                              animate={{ height: isSpeaking ? ["20%", `${h}%`, "20%"] : ["10%", "30%", "10%"] }}
                              transition={{ repeat: Infinity, duration: 0.5 + (i % 5) * 0.1 }}
                              className="w-1.5 bg-gradient-to-t from-sky-500 via-cyan-400 to-blue-300 rounded-full"
                            />
                          ))}
                        </div>
                      )}
                      ...
                      <div className="flex items-center gap-4 mt-3">
                        {!isCalling ? (
                          <button
                            ...
                            className="px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-full font-bold text-xs shadow-lg shadow-emerald-500/30 flex items-center gap-2 transition-transform active:scale-95"
                          >
                          ...
                          <>
                            <button
                              ...
                              className={`p-3 rounded-full text-white transition-all ${
                                isListening ? "bg-red-500 animate-pulse ring-4 ring-red-500/40" : "bg-sky-600 hover:bg-sky-500"
                              }`}
                              title={isListening ? "Listening... Speak into microphone" : "Click to speak via Mic"}
                            >
                            ...
                            <button
                              ...
                              className={`p-3 rounded-full text-white ${isMuted ? "bg-amber-500" : "bg-slate-700 hover:bg-slate-600"}`}
                              title={isMuted ? "Muted" : "Unmute Speech TTS"}
                            >
                            ...
                            <button
                              ...
                              className="px-6 py-3 bg-red-500 hover:bg-red-600 text-white rounded-full font-bold text-xs shadow-lg shadow-red-500/30 flex items-center gap-2 transition-transform active:scale-95"
                            >
                        ...
                        <div className="space-y-2 pt-1">
                            ...
                            <input
                              ...
                              placeholder={
                                voiceLang === "ta"
                                  ...
                                  : "Type input (e.g. 'Hello, how are you?' or 'Book demo')"
                              }
                              className="flex-1 px-4 py-2.5 bg-slate-900 text-white text-xs rounded-xl border border-slate-800 focus:outline-none focus:border-sky-500"
                            />
                            <button
                              type="submit"
                              className="px-4 py-2.5 bg-sky-500 hover:bg-sky-600 text-white font-bold rounded-xl text-xs flex items-center gap-1.5 shadow-md shadow-sky-500/20"
                            >
                          ...
                          <div className="flex flex-wrap gap-2 pt-1">
                            {voiceLang === "ta" ? (
                              <>
                                <button
                                  onClick={() => sendVoiceQuery("வணக்கம், எப்படி இருக்கிறீர்கள்?")}
                                  className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-sky-300 text-xs rounded-lg border border-slate-800 font-semibold"
                                >
                                  🗣️ "வணக்கம், எப்படி இருக்கிறீர்கள்?"
                                </button>
                                <button
                                  onClick={() => sendVoiceQuery("உங்கள் சேவையின் கட்டண விவரங்கள் என்ன?")}
                                  className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-slate-300 text-xs rounded-lg border border-slate-800"
                                >
                              ...
                              <>
                                <button
                                  onClick={() => sendVoiceQuery("Hello, how are you?")}
                                  className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-sky-300 text-xs rounded-lg border border-slate-800 font-semibold"
                                >
                                  🗣️ "Hello, how are you?"
                                </button>
                                <button
                                  onClick={() => sendVoiceQuery("Can I book an appointment for tomorrow?")}
                                  className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-slate-300 text-xs rounded-lg border border-slate-800"
                                >
                  ...
                  <div className="bg-slate-900 text-white rounded-2xl p-6 border border-slate-800 space-y-4">
                    ...
                    <form onSubmit={handleSendChat} className="flex gap-2">
                      <input
                        ...
                        placeholder="Type a WhatsApp query (e.g. 'Show catalog' or 'Hi')..."
                        className="flex-1 px-4 py-2.5 bg-slate-800 text-white text-xs rounded-xl border border-slate-700 focus:outline-none focus:border-emerald-500"
                      />
                      <button
                        type="submit"
                        className="px-4 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-xs font-bold flex items-center gap-1"
                      >
                        <Send className="w-3.5 h-3.5" />
                        Send
                      </button>
                  ...
                  <div className="bg-slate-950 text-white rounded-2xl p-6 border border-slate-800 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                    <div className="md:col-span-6 space-y-4">
                      ...
                      <p className="text-xs text-slate-400 leading-relaxed">
                        Deploy your enterprise mobile app with native push notifications, offline catalog cache, and biometrics authentication out-of-the-box.
                      </p>
                      ...
                      <div className="flex gap-2 pt-2">
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                            appTab === "home" ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-400"
                          }`}
                        >
                          Home Feed
                        </button>
                        <button
                          onClick={() => setAppTab("products")}
                          className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                            appTab === "products" ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-400"
                          }`}
                        >
                          E-Commerce
                        </button>
                        <button
                          onClick={() => setAppTab("notification")}
                          className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                            appTab === "notification" ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-400"
                          }`}
                        >
                          Push Preview
                        </button>
                    ...
                    <div className="md:col-span-6 flex justify-center">
                      <div className="w-56 h-[340px] bg-slate-900 border-4 border-slate-800 rounded-[36px] shadow-2xl p-3 flex flex-col justify-between relative overflow-hidden">
                        ...
                        {appTab === "notification" && (
                          <div className="p-2.5 bg-indigo-950/80 border border-indigo-500/40 rounded-xl text-left space-y-1 my-auto">
    ...
    voice.synthesize(text, wav_file)`}
            ...
            {activeTab === "overview" && (
              <div className="space-y-4">
                ...
                <h4 className="text-sm font-bold text-foreground pt-2">Key Core Features:</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {solution.features.map((feat, idx) => (
                    <div
                      key={idx}
                      className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800"
                    >
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">&lt;50ms Realtime Execution</p>
                  </div>
                  <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
          ...
          <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-wrap items-center justify-between gap-4">
            <button
              onClick={onClose}
              className="px-5 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-200/60 dark:hover:bg-slate-800 rounded-xl transition-colors"
            >
              Close Preview
            </button>
